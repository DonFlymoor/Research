...
local log = log
local pi = math.pi
local visualShaftAngleCoef = 360 / pi
...
local function updateGFX(dt)
  for i = 1, deviceCount, 1 do
    ...
    if device.electricsName and device.visualShaftAngle then --only take care of devices that are meant to have a public angle
      device.visualShaftAngle = (device.visualShaftAngle + device[device.outputAVName] * dt) % pi
...
local function update(dt)
  ...
  --performanceLogger.measureAverage("speeds", 10000)

  --performanceLogger.startMeasurement("torques")
  for i = 1, deviceCount, 1 do
    orderedDevices[i]:torqueUpdate(dt)
  end
  --performanceLogger.measureAverage("torques", 10000)
