...
local function update(dt)
  ...
  if state == "ready" then
    ...
    if throttle > 0 and transbrakeActive then
      state = "countdown"
      countdownTimer = 3
      gui.message("Drag timer armed...", 4, "vehicle.dragtimer.status")
    end
  ...
  elseif state == "countdown" then
    ...
    if countdownTimer <= 0 then
      ...
      countdownTimer = 0
      startingPosition = vec3(obj:getPosition())
      timer = 0
      sixtyFeetTime = 0
      eighthMileTime = 0
      eighthMileSpeed = 0
      quarterMileTime = 0
      quarterMileSpeed = 0
      gui.message("Go!", 1, "vehicle.dragtimer.status")
    end
  ...
  elseif state == "measuring" then
    ...
    local currentVelocity = vec3(obj:getVelocity()):length()
  ...
  elseif state == "done" then
    gui.message("Results are in!", 2, "vehicle.dragtimer.status")
    print("Drag results:")
