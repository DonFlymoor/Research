...
local function updateGFX(dt)
  ...
  if abs(flap - lastFlap) > 0.005 then
    gui.message(string.format("Flaps: %d%%", flap * 100), 1, "vehicle.flaps")
    lastFlap = flap
  end
  if abs(slat - lastSlat) > 0.005 then
    gui.message(string.format("Slats: %d%%", slat * 100), 1, "vehicle.slats")
    lastSlat = slat
  end
end
...
local function toggleTrimMode()
  ...
  gui.message(string.format("Trim mode: %s", currentTrimMode), 5, "vehicle.trimmode")
end
