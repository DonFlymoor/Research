...
local templatingSystem = require "template"
...
local function start(_bindhost, _bindport, _fileroot, _handlers, _argsCallback)
  ...
  tcp_socket:listen()
  log('D', 'simpleHttpServer', "Simple HTTP WebServer running on port "..bindport)
end
...
local function get_file_extension(fn)
  local base, filename, ext = string.match(fn, "(.-)([^/]-([^/%.]+))$")
