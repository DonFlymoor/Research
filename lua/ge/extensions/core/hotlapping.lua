...
-- Wether the hotlapping is allowPlacingCP or not.
...
-- Wether the course should be started instantly or after the first lap
...
local function positionMarkers( )
    ...
    if checkPointCount > 1 then
        scenetree.hotlappingMarkerNext:setPosition((vec3(checkPointPosAndSize[((nextCheckPointToBePassed+1)%checkPointCount)+1].position)):toPoint3F())
        scenetree.hotlappingMarkerNext.hidden = false;
    end
...
local function onUpdate()
    ...
    if not be:getEnabled() then
        if pausedStart == 0 then
            pausedStart = Engine.Platform.getRealMilliseconds()
            guihooks.trigger("HotlappingTimerPause")
        end
        currentPauseTime = Engine.Platform.getRealMilliseconds() - pausedStart
    end
    ...
    if closed and scenetree.hotlappingMarker then 
        ...
        scenetree['markerBase'..nextIndex].instanceColor = ColorF( 1, 0.07, 0, markerAlpha/2 + 0.5):asLinear4F()

        if checkPointCount > 1 then
            local camdistSqt = vec3(checkPointPosAndSize[overNextIndex].position):squaredDistance(camPos)
            local markerAlpha = camdistSqt / 2000
            if markerAlpha > 1 then markerAlpha = 1 end
            scenetree.hotlappingMarkerNext.instanceColor = ColorF( 1, 0.4, 0.1, markerAlpha * 0.9 + 0.1):asLinear4F()
            scenetree['markerBase'..overNextIndex].instanceColor = ColorF( 1, 0.85, 0.4, markerAlpha/2 + 0.5):asLinear4F()
        end
    end

    lastRealMillis = Engine.Platform.getRealMilliseconds()
end

local function passTimeToGUI()
  local dt = Engine.Platform.getRealMilliseconds() - lastRealMillis
...
local function changeSize( sign,  all )
    if all then
        ...
        for k,v in pairs(checkPoints) do
            v:setScale(Point3F(4 * size,4*size,8))
            scenetree['markerBase'..i]:setScale(Point3F(6.8 * size,6.8*size,7))
            i = i+1
        end
    else
        -- change on CP size
