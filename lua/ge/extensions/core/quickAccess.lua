...
local function registerDefaultMenus()
  ...
  addEntry({ level = '/manage/', generator = function(entries)
    if be:getObjectCount() > 0 then
      table.insert(entries,{ level = '/manage/', title = 'ui.radialmenu2.Manage.Remove', icon='material_delete_forever', onSelect = function() core_vehicles.removeCurrent(); return {'hide'} end} )
      table.insert(entries,{ level = '/manage/', title = 'ui.radialmenu2.Manage.Clone', icon='radial_clone', onSelect = function() core_vehicles.cloneCurrent(); return {'hide'} end} )
    end
  ...
  addEntry({ level = '/switch_vehicles/', icon = 'radial_switch', generator = function(entries)
    ...
    local vid = -1 -- matches all
    if lveh ~= nil then vid = lveh:getID() end
    ...
    for i = 0, be:getObjectCount() do
      ...
      if veh and veh:getID() ~= vid then
        ...
        if FS:fileExists(filePath) then --check for main info
          ...
          --print("vehicleName="..vehicleName.."  vehConfig="..dumps(vehConfig))
        end
  ...
  addEntry({ level = '/manage/', title = 'ui.radialmenu2.Manage.Select', icon='material_directions_car', onSelect = function() guihooks.trigger('ChangeState', {state = 'menu.vehicles'}) ; return {'hideMeOnly'} end} )
...
local function show(level)
  ...
  if veh then
    ...
    veh:queueLuaCommand('extensions.core_quickAccess.requestItems("' .. tostring(currentLevel) .. '")')
...
local function setEnabled(enabled)
  if enabled then
    ...
    show()
    guihooks.trigger('quickAccessEnabled', enabled)
  else
    hide()
    guihooks.trigger('quickAccessEnabled', enabled)
  end
...
local function moved()
  lastTimeMoved = Engine.Platform.getRealMilliseconds()
end
...
local function onVehicleFocusChanged(args)
...
M.onUpdate = onUpdate
M.onVehicleFocusChanged = onVehicleFocusChanged
