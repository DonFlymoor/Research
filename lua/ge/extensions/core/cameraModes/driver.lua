...
function C:onSettingsChanged()
  ...
  self.fov = settings.getValue('cameraDriverFov') -- ignore jbeam fov, use user FOV instead
  self.fovOffset = 0
end
...
function C:update(data)
  ...
  local player = 0
  local camNodeID, rightHandDrive = core_camera.getDriverData(player)
  ...
  if mustNotifyFov then
    ...
    self.saveTimeout = 1
    ui_message({txt='ui.camera.fovDriver', context={degrees=fov}}, 2, 'cameramode')
  end
  ...
  if self.allowSeatAdjustments then
    dr = dxSmoother:get(MoveManager.left     - MoveManager.right  , data.dt)
  ...
  if self.saveTimeout and self.saveTimeout <= 0 then
    ...
    settings.setValue('cameraDriverFov', fov)
    self.saveTimeout = nil
  end
end
