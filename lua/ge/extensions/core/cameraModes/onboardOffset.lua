...
function C:update(data)
  ...
  if Xinput > triggerValue then
    self.camRot.x = Xinput * 1600
    -- self.globalOffset = vec3((3.5 * (-Xinput)), (1.5 * (-Xinput)), 0)
  elseif Xinput < -triggerValue then
    self.camRot.x = Xinput * 1600
    -- self.globalOffset = vec3((4 * (-Xinput / 1)), (2 * (-Xinput / 1)), 0)
  ...
  -- Angle used to define where offset starts being applied to camera (looking out window/through rear window)
  local angle = 120

  if self.camRot.x > angle then
    self.globalOffset = vec3(((angle-self.camRot.x) / 100), 0, 0)
  end
