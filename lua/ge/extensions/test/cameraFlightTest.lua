...
local function saveTestsToJsonFile()
	...
	jsonWriteFile1(fileName, allTestResults)
end
...
local function compareTests(newTest, oldTest, resultFilename)
	...
	jsonWriteFile1(resultFilename, allComparisonResults)
end

local function compareTestsFromJsonFile(file1, file2, resultFilename)
	local test1 = jsonReadFile(file1)
	if not test1 then
		log('W', logTag, "No json-file \"" .. file1 .. "\" found.")
		return
	end

	local test2 = jsonReadFile(file2)
	if not test2 then
...
local function loadLevel(levelname)
	...
	core_levels.startLevel('levels/'.. levelname ..'/main')
end
...
local function endTest()
	...
	--if mapCounter > 1 then
		...
		compareTestsFromJsonFile("performanceTest.json", "15DayOldPerformanceTest.json", "TestComparison15Days.json")
		compareTestsFromJsonFile("performanceTest.json", "20DayOldPerformanceTest.json", "TestComparison20Days.json")
		compareTestsFromJsonFile("performanceTest.json", "30DayOldPerformanceTest.json", "TestComparison30Days.json")
		compareTestsFromJsonFile("performanceTest.json", "60DayOldPerformanceTest.json", "TestComparison60Days.json")
		compareTestsFromJsonFile("performanceTest.json", "90DayOldPerformanceTest.json", "TestComparison90Days.json")
...
local function onClientStartMission()
	...
	settings.setState({vsync = false}, true)
