...
local function computeStats(stats)
  ...
  for k,v in ipairs(stats) do
    ...
    local b = a*a; a = b*b; b = a*a; a = b*b; b = a*a -- cheap math.power
    ...
    local r = 100*smoothstep(math.abs(v.stopDistance.fill)/8)
    if r < 1 then r = 0 end
...
local function success(reason)
  if false then -- temporarily disabled bus stats, #2783
    ...
    local playerVehicle = scenetree.findObject(playerInstance)
    statistics_statistics.initialiseArbitraryStat(   "busDamage",        "Damage", playerVehicle, playerInstance, 100, 100)
    statistics_statistics.initialiseArbitraryStat(  "smoothness",    "Smoothness", playerVehicle, playerInstance, 100, 100)
    statistics_statistics.initialiseArbitraryStat(   "stopAngle",    "Stop Angle", playerVehicle, playerInstance, 100, 100)
    statistics_statistics.initialiseArbitraryStat("stopDistance", "Stop Distance", playerVehicle, playerInstance, 100, 100)
...
local function onPreRender(dt, dtSim)
  ...
  if running and dtSim > 0 then
    ...
    local smAcc = smootherAcc:getUncapped(acc.x, dtSim) / core_environment.getGravity()
    ...
    local smoothness = math.max(math.abs(smAcc) + smJer)
    ...
    stats[stop].smoothness.fill = math.max(smoothness, stats[stop].smoothness.fill)
    prevVel = currVel
    prevPos = currPos
    prevAcc = currAcc
    ...
    -- stop angle rating
    local dotAngle = 0
    if math.abs(stopDistance) < 20 then
      local fwd = vec3(pv:getDirectionVector()):normalized()
      ...
      dotAngle = math.abs(fwdAligned:dot(busStopFwd))
    ...
    data.busStopping = math.abs(stopDistance) < 20
    if false then -- temporarily disabled bus stats, #2783
...
local function onBusUpdate(state)
  ...
  if state.event == "onTriggerTick" and not stopComplete and nextStop and nextStop[1] == state.triggerName then
    ...
    elseif stopTimer < timeToWaitAtStop then
      ...
      guihooks.trigger('ScenarioRealtimeDisplay', {msg = "scenarios.busRoutes.wait", context = {remaining=tostring(math.ceil(timeToWaitAtStop - stopTimer))}})
