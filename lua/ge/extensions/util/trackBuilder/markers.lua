...
local function addLeftMeshMarker(list)
  ...
  marker:setField('shapeName', 0, "art/shapes/interface/track_editor_marker.dae")
  marker:setPosition(Point3F(0,0,0))
  marker.scale = Point3F(2.25, 0.1, 2.25)
  marker:setField('rotation', 0, '0 0 0 1')
  marker.useInstanceRenderData = true
  marker:setField('instanceColor', 0, '1 0 1 0.66')
  marker:setField('collisionType', 0, "Collision Mesh")
...
local function addCeilingMeshMarker(list)
  ...
  marker:setField('shapeName', 0, "art/shapes/interface/track_editor_marker.dae")
  marker:setPosition(Point3F(0,0,0))
  marker.scale = Point3F(1.5, 0.1, 5)
  marker:setField('rotation', 0, '0 0 0 1')
  marker.useInstanceRenderData = true
...
local function nodeGetter(name,allNodes)
  local list = {}
  for i = 1, #allNodes do
    if allNodes[i][name] ~= nil then
      list[#list+1] = allNodes[i]
    end
  end
  return list
end
...
local function transformBankMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    local quat = convertQuatToTorqueFormat(rot)

    markers['bank'][i].scale =  Point3F(0.1, 5, 2.5)
    markers['bank'][i]:setPosition(node.markerInfo.position:toPoint3F())
...
local function transformHeightMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    markers['height'][i]:setPosition((node.markerInfo.position + vec3(0,0,-1)):toPoint3F())
    markers['height'][i]:setScale(Point3F(1,1, node.markerInfo.position.z -1))
  end
end
...
local function transformWidthMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['width'][i*2-1]:setPosition((node.markerInfo.position + right ):toPoint3F())
    markers['width'][i*2-1].scale = Point3F(2,2,2)

    markers['width'][i*2]:setPosition((node.markerInfo.position + left):toPoint3F())
    markers['width'][i*2].scale = Point3F(2,2,2)
  end
end
...
local function transformCenterMeshMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['centerMesh'][i].scale =  Point3F(2,0.1,2)
    markers['centerMesh'][i]:setPosition((node.markerInfo.position + down):toPoint3F())
...
local function transformLeftMeshMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['leftMesh'][i].scale =  Point3F(2,0.1,2)
    markers['leftMesh'][i]:setPosition((node.markerInfo.position + downLeft):toPoint3F())
...
local function transformRightMeshMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['rightMesh'][i].scale =  Point3F(2,0.1,2)
    markers['rightMesh'][i]:setPosition((node.markerInfo.position + downRight):toPoint3F())
...
local function transformCheckpointMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['checkpoint'][i].scale =  Point3F(node.checkpointSize,node.checkpointSize, node.checkpointSize)
    markers['checkpoint'][i]:setPosition((node.markerInfo.position + off):toPoint3F())
...
local function transformLeftWallMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['leftWall'][i].scale =  Point3F(1,0.1,3)
    markers['leftWall'][i]:setPosition((node.markerInfo.position + downLeft):toPoint3F())
...
local function transformRightWallMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['rightWall'][i].scale =  Point3F(1,0.1,3)
    markers['rightWall'][i]:setPosition((node.markerInfo.position + downRight):toPoint3F())
...
local function transformCeilingMeshMarkers(nodes) 
  ...
  for i, node in ipairs(nodes) do
    ...
    markers['ceilingMesh'][i].scale =  Point3F(1,0.1,3)
    markers['ceilingMesh'][i]:setPosition((node.markerInfo.position + upCenter):toPoint3F())
...
local function hideMarkers(name)
  for i = 1,#markers[name] do
    if markers[name][i] ~= nil then
      markers[name][i].scale = Point3F(0,0,0)
    end
  end
end
