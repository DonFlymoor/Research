...
local function normalizeFiles(fileList, sourceFile)
  ...
  local dir, filename, ext = string.match(sourceFile, "(.-)([^/]-([^%.]+))$")
  for k, fn in pairs(fileList) do
    local dir2, filename2, ext2 = string.match(fn, "(.-)([^/]-([^%.]+))$")
    if dir2 == '' then
      fileList[k] = dir .. fn
    end
  end
...
local function processSimObject(n, sourceFile)
  ...
  if tableSize(entity.deps) == 0 then entity.deps = nil end
  if tableSize(entity.provides) == 0 then entity.provides = nil end
