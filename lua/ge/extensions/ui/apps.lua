...
local currentVersion = 0.44
local appsDir = 'ui/modules/apps'
local logTag = 'ui-apps'
...
local function getList()
  ...
  for _, path in ipairs(jsonFiles) do
    ...
    if appDir then
      local appData = jsonReadFile(path)
...
local function resetLayout (gamestate)
  if FS:fileExists(userLayoutsFile) and gamestate ~= nil then
    local default = jsonReadFile(defaultLayoutsFile)
    local user = jsonReadFile(userLayoutsFile)

    user[gamestate] = default[gamestate]
    local worked = jsonWriteFile(userLayoutsFile, user, true)
...
local function resetLayouts()
  ...
  return jsonReadFile(defaultLayoutsFile)
end

local function getSettings(appDir)
  return jsonReadFile('/ui/modules/apps/'..appDir..'/settings.json') or nil
end
...
local function getLayouts()
  local userLayouts    = jsonReadFile(userLayoutsFile)
  local defaultLayouts = jsonReadFile(defaultLayoutsFile)
...
local function saveLayouts(layouts_data)
  ...
  jsonWriteFile(userLayoutsFile, layouts_data, true)
end
...
-- === ui2 =============================================================================
local function getList2 (baseFolder, searchFolder)
  if baseFolder == nil then
    baseFolder = 'ui2/drive/'
  end

  if searchFolder == nil then
    searchFolder = baseFolder .. 'Content/Apps/'
  end

  local jsonFiles = FS:findFilesByRootPattern(searchFolder, 'info.json', 1, false, false)
  local appDirRegex = searchFolder .. '([%w|_|%-|%s]+)/info.json'
  local data = {}

  for _, path in ipairs(jsonFiles) do
    local folderName = string.match(path, appDirRegex) or ''
    local folderPath = searchFolder .. folderName
    local uiFolderPath = string.gsub(folderPath, baseFolder, '')
    data[folderName] = {}
    ...
    for key, path in ipairs(data[folderName].pngFiles) do
      data[folderName].pngFiles[key] = string.gsub(path, baseFolder, '')
    end
    data[folderName].cssFiles = FS:findFilesByRootPattern(folderPath, 'app*.css', 1, false, false)
    for key, path in ipairs(data[folderName].cssFiles) do
      data[folderName].cssFiles[key] = string.gsub(path, baseFolder, '')
    end
    if FS:fileExists(folderPath .. '/data.json') then
      data[folderName].jsonFile = uiFolderPath .. '/data.json'
    end
    if FS:fileExists(folderPath .. '/app.js') then
      data[folderName].jsFile = uiFolderPath .. '/app.js'
    end
  end
...
local function sendList2 (baseFolder, searchFolder)
  guihooks.trigger('AppList', getList2(baseFolder, searchFolder))
end
...
local function getPresets2 ()
  ...
  local res = {}
  local files = FS:findFilesByRootPattern( 'settings/', 'app-preset*.json', 1, false, false)
  for _, v in pairs(files) do
    if v == 'settings/app-preset.json' then
      presets.custom = jsonReadFile(v)
      for k in pairs(presets.custom) do
        presets.custom[k].source = 'custom'
      end
    elseif v == 'settings/app-preset-default.json' then
      res = jsonReadFile(v)
      for k in pairs(res) do
        res[k].source = 'default'
      end
    else
      presets.mods[v] = jsonReadFile(v)
...
local function sendPresets2 ()
  guihooks.trigger('appPresets', getPresets2())
end
...
local function savePresets2 (presets)
  ...
  jsonWriteFile('settings/app-preset.json', res, true)
  return res
end
