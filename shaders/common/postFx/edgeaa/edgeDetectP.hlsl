...
float GetEdgeWeight(float2 uv0, in sampler2D prepassDepthBuffer, in sampler2D prepassBuffer, in float2 targetSize)
{
   float2 offsets[9] = {
      float2( 0.0,  0.0),
      float2(-1.0, -1.0),
   ...
   for(int i = 0; i < 9; i++)
   {
      ...
      float4 gbSample = decodeGBuffer( prepassDepthBuffer, prepassBuffer, uv, projParams );
      Depth[i] = gbSample.a;
