...
uniform_sampler2D( colorSampler , 0 ); // Original source image  
uniform_sampler2D( smallBlurSampler , 1 ); // Output of SmallBlurPS()  
uniform_sampler2D( largeBlurSampler , 2 ); // Blurred output of DofDownsample()
uniform_sampler2D( prepassSampler , 3 ); // 
uniform_sampler2D( prepassDepthSampler , 4 );
...
{  
   ...
   sum += weight * (half3)tex2Doffset( colorSampler, tc, float2( +0.5, -1.5 ) ).rgb;  
   sum += weight * (half3)tex2Doffset( colorSampler, tc, float2( -1.5, -0.5 ) ).rgb;  
   sum += weight * (half3)tex2Doffset( colorSampler, tc, float2( -0.5, +1.5 ) ).rgb;  
   sum += weight * (half3)tex2Doffset( colorSampler, tc, float2( +1.5, +0.5 ) ).rgb;  
   return sum;  
}  
...
half4 main( PFXVertToPix IN ) : SV_TARGET
{  
   ...
   //nearCoc = 0;
   depth = (half)decodeGBuffer( prepassDepthSampler, prepassSampler, IN.uv3, projParams ).w;  
