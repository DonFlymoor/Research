...
uniform_sampler2D( colorSampler, 0);
uniform_sampler2D( prepassSampler, 1);
uniform_sampler2D( prepassDepthSampler, 2);
...
float processCoC4Depth( in int i, in Pixel IN, in float2 rowOfs[4] )
{
    float4 depth;
    depth[0] = decodeGBuffer( prepassDepthSampler, prepassSampler, float2( IN.tcDepth0.xy + rowOfs[i] ), projParams ).w;
    depth[1] = decodeGBuffer( prepassDepthSampler, prepassSampler, float2( IN.tcDepth1.xy + rowOfs[i] ), projParams ).w;
    depth[2] = decodeGBuffer( prepassDepthSampler, prepassSampler, float2( IN.tcDepth2.xy + rowOfs[i] ), projParams ).w;
    depth[3] = decodeGBuffer( prepassDepthSampler, prepassSampler, float2( IN.tcDepth3.xy + rowOfs[i] ), projParams ).w;
    return (float)clamp( dofEqWorld.x * depth + dofEqWorld.y, 0.0, maxWorldCoC );
}

half4 main( Pixel IN ) : SV_Target
{  
   ...
   coc[0] = (half)processCoC4Depth(0, IN, rowOfs);
   coc[1] = (half)processCoC4Depth(1, IN, rowOfs);
   coc[2] = (half)processCoC4Depth(2, IN, rowOfs);
   coc[3] = (half)processCoC4Depth(3, IN, rowOfs);
