  ...
  uiElements: {
    graphics: {
      ...
      checkboxGroup2: [
        ...
        { name: 'ui.options.graphics.GraphicPostfxQuality',   key: 'GraphicPostfxQuality',   customOnly: false},
        { name: 'ui.options.graphics.GraphicAnisotropic',     key: 'GraphicAnisotropic',     customOnly: false}
      ],

      dynReflectionSliders: [
        { name: 'Texture Size',     key: 'GraphicDynReflectionTexsize',        min: 0,  max: 3,    step: 1   },
        { name: 'Faces Per Update', key: 'GraphicDynReflectionFacesPerupdate', min: 1,  max: 6,    step: 1   },
        { name: 'Detail',           key: 'GraphicDynReflectionDetail',         min: 0,  max: 1,    step: 0.1 },
        { name: 'Distance',         key: 'GraphicDynReflectionDistance',       min: 10, max: 1000, step: 10, unitsTxt: 'm' }
      ]
    },
  ...
  return {
    template: `
      <span style="display: inline-block;" layout="row" layout-align="center center">
        <kbd ng-class="{light: !dark, dark: dark}" style="font-size: 0.9em;" ng-if="viewerObj && !viewerObj.special">
          <span layout="row" layout-align="center center">
            <md-icon class="material-icons bng-binding-icon" style="font-size: 1.1em;" ng-style="{color: (dark ? 'white' : 'black')}">{{ viewerObj.icon }}</md-icon>
            <span ng-hide="hideName">{{ viewerObj.control || uppercase }}</span>
          </span>
        </kbd>
...
.directive('hardwareKeyControl', function () {
  return {
    template: `<span ng-repeat="ctrl in activeControls"><binding dark="true" key="ctrl" device="dev" style="margin: 5px; font-size:18px;"></binding></span>`,
    replace: true,
    link: function (scope, element, attrs) {
  ...
  return {
    ...
    <md-list>
      ...
      <md-list-item layout ng-if="data.details.action == 'steering'">
        ...
        <md-slider ng-model="data.details.angle" ng-change="inputResponseCurveRender()" flex min="0" max="1200" step="1" aria-label="_"></md-slider>
        <md-input-container class="bng-controls-aux-input" >
          <input aria-label="_" type="number" min="00" max="1200" step="1" ng-model="data.details.angle" ng-change="inputResponseCurveRender()">
        </md-input-container>
      </md-list-item>
...
.factory('ControlsUtils', ['$filter', '$log', '$q', '$rootScope', 'bngApi', 'controlsContents', function ($filter, $log, $q, $rootScope, bngApi, controlsContents) {
  ...
  $rootScope.$on('ControllersChanged', function (event, data) {
    $rootScope.$evalAsync(function () {
      ...
      // vm.ffbCapable = service.isFfbCapable();
    });
  });
  ...
  bngApi.engineLua('bindings.notifyUI("controls utils service needs the data")');
  ...
  var service = {
    ...
    removeBinding: function (device, control, action, mockSave) {
      ...
      if (mockSave) {
        ...
        bngApi.engineLua(`bindings.saveBindingsToDisk(${bngApi.serializeToLua(deviceContents)})`);
      }
    },
    ...
    resetAllBindings: function () {
      bngApi.engineLua('bindings.resetAllBindings()');
    },
