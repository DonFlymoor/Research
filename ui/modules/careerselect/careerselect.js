(function() {
  ...
  **/
  .controller('CareerSelectController', ['$filter', 'logger', '$scope', '$state', '$interval', 'bngApi', 'Aux',
    function($filter, logger, $scope, $state, $interval, bngApi, Aux) {
    ...
    bngApi.engineLua('campaign_campaignsLoader.getList()', function(res) {
      $scope.filtered = [];
      // logger.debug('[CareerSelectController] got scenarios', JSON.stringify(res, null, '  '));
      // logger.debug('[CareerSelectController] got scenarios %o', res);
      ...
      availableCareers = res;
      
      $scope.$watchGroup(['query', 'selectedOrder'], function (value) {
        var temp = $filter('filter')(availableCareers, {__displayName__: value[0]});
    ...
    $scope.selectCareer = function(campaign) {
      $scope.selected = campaign;
    };

    $scope.startCareer = function(campaign) {
