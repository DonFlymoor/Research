...
.config(['$compileProvider', '$logProvider', 'loggerProvider', '$stateProvider', '$urlRouterProvider', '$mdThemingProvider', '$translateProvider', 'AppDefaults', 'toastrConfig', '$provide',
  ...
  .state('menu', {
    ...
    .state('menu.vehicleconfig', {
      url: '/vehicle-config',
      templateUrl: 'modules/vehicleconfig/vehicleconfig.new.html',
      controller: 'VehicleconfigController as vehicleconfig'
    })
  ...
  .state('tutorial', {
    url: '/tutorial',
    templateUrl: 'modules/tutorial/intro.html',
    controller: 'TutorialIntroController'
  })
  ...
  if(!beamng.shipping) {
    ...
    .state('terraingen', {
      url: '/terraingen',
      templateUrl: 'modules/terraingen/terraingen.html',
      controller: 'TerrainGenController as terraingen'
    })
    .state('luadebug', {
      url: '/luadebug',
      templateUrl: 'modules/luadebug/luadebug.html',
      controller:  'LuaDebugController as luadebug'
    })
    .state('imageviewer', {
      url: '/imageviewer?useKeys',
      templateUrl: 'modules/imageviewer/imageviewer.html',
      controller: 'ImageviewerController as imgview'
    })
...
function ($animate, $http, logger, $rootScope, $templateCache, $window, $translate,  AppLayout, Aux, bngApi, Settings, SettingsAuxData) {
  ...
  $http.get('assets/svg-symbols.svg')
  .success(svgSprite => {
    ...
    $rootScope.$on('MenuItemNavigation', function (event, action, val) {
      ...
      if (useCrossfire) {
        if(action == 'confirm') {
          ...
          collectRects(scope || $rootScope);
          navigate(scope || $rootScope, action);
...
$scope.$on('requestUIInitialised', () => {
  ...
  vm.menuEntries = {
    freeroam: [
      ...
      { translateid: 'ui.dashboard.performance',   icon: 'equalizer',              state: 'performance',     advanced: true },
      { translateid: 'ui.dashboard.audiodebug',    icon: 'headset',                state: 'menu.audiodebug', advanced: true },
      { translateid: 'ui.dashboard.feedback',      icon: 'gesture',                state: 'feedback', advanced: true },
      { translateid: 'ui.dashboard.mainmenu',      icon: 'keyboard_backspace', state: '.', action: () => $timeout(() => vm.quit()), bottom: true},
    ],
  ...
  vm.sections = { // defines in which state after which icons should be a divider
    freeroam: ['.', 'menu.help', 'menu.vehicleconfig', 'menu.environment', 'performance', 'photomode', 'menu.options.graphics'],
  ...
  $scope.$watch('app.showMenu', function (newv, oldv) {
    bngApi.engineLua("bindings.menuActive("+newv+")");
  ...
  $scope.$on('MenuOpenModule', function (event, data) {
    ...
    case 'vehicleselect':
      ...
      break;
    case 'vehicleconfig':
      $state.go('menu.vehicleconfig').then(function () {vm.showMenu = true; });
      break;
    case 'photomode':
  ...
  (function () {
    ...
    window.closure = {$injector: $injector, state: $state, vm: vm, bngApi: bngApi, translate: $translate, scope: $scope, stateNav: SimpleStateNav};
