...
local function scenarioJudge(scoreCard)
  ...
  scorePoints = clamp((scoreCard / 100), 0, maxDamageDonePoints)
  data = {
    points=scorePoints,
    value=scoreCard
  }
...
local function startAIVehicle()
    ...
    local arg = {vehicleName = 'scenario_targetcar', 
                waypoints = {'crush_aiwp1', 'crush_aiwp2', 'crush_aiwp3', 'crush_aiwp4', 'crush_aiwp5', 'crush_aiwp6'}, 
                routeSpeed = 50/3.6,
...
local function onRaceWaypointReached(data)
  ...
  if data.waypointName == AIStopWaypointName then
    stopAIVehicle()
  end
