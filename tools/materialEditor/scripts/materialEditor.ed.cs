...
function MaterialEditorGui::establishMaterials(%this)
{
    ...
    singleton ArrayObject(materialEd_CustomTexArray);
}
...
function MaterialEditorGui::open(%this)
{
    ...
    // Additional things
    MaterialEditorPropertiesWindow-->annotationPopUp.clear();
    ...
    // add annotations to drop down
    %annotations = getObjectAnnotationsWithColors();
    %count = getFieldCount(%annotations);
    MaterialEditorPropertiesWindow-->annotationPopUp.add("|0|0|0");
    for (%i = 0; %i < %count; %i++) {
        %name = getField(%annotations, %i);
        MaterialEditorPropertiesWindow-->annotationPopUp.add(%name);
    }
    ...
    if(false) {
	    MaterialEditorTextureStack.clear();
	    materialEd_CustomTexArray.empty();
	    %this.addTexture("metalness");
	    %this.addTexture("roughness");
	    %this.addTexture("ambientOcclusion");
	    %this.addTexture("colorPalette");
	    %this.addTexture("emmisive");
    }
}
...
function SubMaterialSelector::onSelect( %this )
{
    ...
    if( !isObject( %material ) )
    {
        ...
        MaterialEditorGui.currentObject.changeMaterial( SubMaterialSelector.text, "", %material );
...
function MaterialEditorGui::setMaterialDirty(%this)
{
    ...
    if( MaterialEditorGui.isMatEditorMaterial( MaterialEditorGui.currentMaterial ) )
    {
        if( !MaterialEditorGui.currentMaterial.isAutoGenerated() )
        {
            ...
            //creating toPath
            %savePath = filePath( %shapePath ) @ "/materials.cs";
...
function MaterialEditorGui::convertTextureFields(%this)
{
    ...
    for(%annotationI = 0; %annotationI < 4; %annotationI++)
    {
        %annotationMap = MaterialEditorGui.currentMaterial.annotationMap[%annotationI];
        %annotationMap = MaterialEditorGui.searchForTexture(MaterialEditorGui.currentMaterial, %annotationMap);
        MaterialEditorGui.currentMaterial.annotationMap[%annotationI] = %annotationMap;
    }
    ...
    for(%reflI = 0; %reflI < 4; %reflI++)
    {
        %reflectivityMap = MaterialEditorGui.currentMaterial.reflectivityMap[%reflI];
        %reflectivityMap = MaterialEditorGui.searchForTexture(MaterialEditorGui.currentMaterial, %reflectivityMap);
        MaterialEditorGui.currentMaterial.reflectivityMap[%reflI] = %reflectivityMap;
    }

    for(%pbrI = 0; %pbrI < 4; %pbrI++)
    {
        %PBRDataMap = MaterialEditorGui.currentMaterial.PBRDataMap[%pbrI];
        %PBRDataMap = MaterialEditorGui.searchForTexture(MaterialEditorGui.currentMaterial, %PBRDataMap);
        MaterialEditorGui.currentMaterial.PBRDataMap[%pbrI] = %PBRDataMap;
    }
}
...
function MaterialEditorGui::guiSync( %this, %material )
{
    ...
    {
        MaterialEditorPropertiesWindow-->annotationMapNameText.setText( (%material).annotationMap[%layer] );
        MaterialEditorPropertiesWindow-->annotationMapDisplayBitmap.setBitmap( (%material).annotationMap[%layer] );
    }
    ...
    if((%material).reflectivityMap[%layer] $= "")
    {
        MaterialEditorPropertiesWindow-->reflectivityMapNameText.setText( "None" );
        MaterialEditorPropertiesWindow-->reflectivityMapDisplayBitmap.setBitmap( "tools/materialeditor/gui/unknownImage" );
    }
    else
    {
    ...
    for(%i = 0; %i < materialEd_CustomTexArray.count(); %i++) {
        %id = materialEd_CustomTexArray.getKey(%i);

        if((%material).getFieldValue(%id @ "Map", %layer) $= "") {
            MaterialEditorPropertiesWindow.findObjectByInternalName(%id @ "MapNameText", true).setText( "None" );
            MaterialEditorPropertiesWindow.findObjectByInternalName(%id @ "MapDisplayBitmap", true).setBitmap( "tools/materialeditor/gui/unknownImage" );
        }
        else {
            MaterialEditorPropertiesWindow.findObjectByInternalName(%id @ "MapNameText", true).setText( (%material).getFieldValue(%id @ "Map", %layer) );
            MaterialEditorPropertiesWindow.findObjectByInternalName(%id @ "MapDisplayBitmap", true).setBitmap( (%material).getFieldValue(%id @ "Map", %layer) );
        }
    }
    ...
    %idx = MaterialEditorPropertiesWindow-->annotationPopUp.findText( (%material).annotation );
    MaterialEditorPropertiesWindow-->annotationPopUp.setSelected(%idx, false);
...
function MaterialEditorGui::updateActiveMaterialName(%this, %name)
{
    ...
    //update mapTO
    addMaterialMapping( MaterialEditorGui.currentMaterial.mapTo, %name );
}
...
function MaterialEditorGui::updateAnnotation(%this, %annotation)
{
    %this.updateActiveMaterial("annotation", %annotation);
}
...
function MaterialEditorGui::addTexture(%this, %texId)
{
    %gui = new GuiContainer(){ // Normal Map
        profile="ToolsGuiDefaultProfile";
        isContainer = "1";
        position = "0 0";
        Extent = "185 54";
        HorizSizing = "width";
        internalName = %texId @ "MapContainer";

        new GuiBitmapCtrl() {
            canSaveDynamicFields = "0";
            internalName = %texId @ "MapDisplayBitmap";
            Enabled = "1";
            isContainer = "0";
            Profile = "ToolsGuiDefaultProfile";
            HorizSizing = "right";
        ...
        new GuiBitmapButtonCtrl() {
            ...
            canSave = "1";
            Visible = "1";
            Command = "MaterialEditorGui.updateTextureMap(\"" @ %texId @ "\", 1);";
            tooltipprofile = "ToolsGuiDefaultProfile";
            ToolTip = "Change the active " @ texId @ " Map for this layer.";
            hovertime = "1000";
            groupNum = "-1";
        ...
        new GuiTextCtrl() {
            canSaveDynamicFields = "0";
            internalName = %texId @ "MapNameText";
            Enabled = "1";
            isContainer = "0";
        ...
        new GuiBitmapButtonCtrl() {
            ...
            canSave = "1";
            Visible = "1";
            Command = "MaterialEditorGui.updateTextureMap(\"" @ %texId @ "\", 0);";
            hovertime = "1000";
            groupNum = "-1";
            buttonType = "PushButton";
            useMouseEvents = "0";
    ...
    MaterialEditorTextureStack.add(%gui);    
    materialEd_CustomTexArray.push_back(%texId, %texId);
}
