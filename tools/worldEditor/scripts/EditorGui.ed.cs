...
function EditorGui::init(%this) {
    ...
    */
    ...
    if (%initialEditor $= "") %initialEditor = "WorldEditorInspectorPlugin";
...
function EditorGui::readCameraSettings(%this, %levelName) {
    if (%levelName !$= %this.levelName) return;
...
function EditorGui::shutdown(%this) {
    %this.writeWorldEditorSettings(); // Store settings.
    %this.setEditor(""); // Deactivate current editor.
    foreach (%plugin in EditorPluginSet) %plugin.onWorldEditorShutdown(); // Call the shutdown callback on the editor plugins.
}

/// This is used to add an editor to the Editors menu which will take over the default world editor window.
function EditorGui::addToEditorsMenu(%this, %displayName, %accel, %newPlugin) {
    ...
    for (%i = 0; %i < %count; %i++) {
        ...
        if (%newPlugin $= %existingPlugins) %alreadyExists = true;
    }

    if (%accel $= "" && %count < 9) %accel = "F" @ %count + 1;
    else %accel = "";

    if (!%alreadyExists) %windowMenu.addItem(%count, %displayName TAB %accel TAB %newPlugin);
...
function EditorGui::setDisplayType(%this, %type) {
    ...
    // Store the current camera rotation so we can restore it correctly when switching back to perspective view
    if (%gui.getDisplayType() == $EditTSCtrl::DisplayTypePerspective)
        %this.lastPerspectiveCamRotation = Game.camera.getRotationEuler();

    %gui.setDisplayType(%type);

    if (%gui.getDisplayType() == $EditTSCtrl::DisplayTypePerspective)
        Game.camera.setRotation(%this.lastPerspectiveCamRotation);
...
function EditorGui::setEditor(%this, %newEditor, %dontActivate) {
    if (isObject(%this.currentEditor)) {
        if (isObject(%newEditor) && %this.currentEditor.getId() == %newEditor.getId()) return;
        if (%this.currentEditor.isActivated) %this.currentEditor.onDeactivated();
        if (isObject(%this.currentEditor.editorGui)) %this.currentOrthoFOV = %this.currentEditor.editorGui.getOrthoFOV();
    }
    ...
    if (%newEditor.isMethod("setEditorFunction")) {
        if (%newEditor.setEditorFunction()) {
            ...
            if (!%dontActivate) %this.currentEditor.onActivated();
        } else {
...
function EditorGui::syncEditor(%this, %newEditor, %newEditorFailed) {
    ...
    if (%newEditorFailed && EWToolsToolbar.isDynamic) {
        if (EWToolsToolbar.isClosed) EWToolsToolbar.reset();
...
function EditorGui::onWake(%this) {
    ...
    while(strpos(%missionFile, "/", %slashPos) != -1) {
        %slashPos = strpos(%missionFile, "/", %slashPos) + 1;
    }
    %levelName = getSubStr(%missionFile , %slashPos , 99);

    if (%levelName !$= %this.levelName) %this.onNewLevelLoaded(%levelName);
    if (isObject(DemoEditorAlert) && DemoEditorAlert.helpTag<2) Canvas.pushDialog(DemoEditorAlert);
...
function EditorGui::onSleep(%this) {
    ...
    foreach (%plugin in EditorPluginSet) %plugin.onEditorSleep();
    if (isObject($Server::CurrentScene)) $Server::CurrentScene.open();
...
function EditorGui::onClientEndMission(%this) {
    foreach (%plugin in EditorPluginSet) {
        if(%plugin.isMethod("onClientEndMission")) {
            %plugin.onClientEndMission();
        }
    }
}
...
function EditorMissionCleanup::onRemove(%this) {
    EditorGui.levelName = "";
    foreach (%plugin in EditorPluginSet) %plugin.onExitMission();
}
...
function EditorGui::removeCameraBookmark(%this, %name) {
    if (!isObject(CameraBookmarks)) return;
...
function EditorGui::removeCameraBookmarkIndex(%this, %index) {
    if (!isObject(CameraBookmarks)) return;
    if (%index < 0 || %index >= CameraBookmarks.getCount()) return;
...
function EditorGui::jumpToBookmark(%this, %name) {
    ...
    Game.camera.setTransformF(%mark.getTransformF());
    return;
}
...
function EditorGui::jumpToBookmarkIndex(%this, %index) {
    ...
    %trans = CameraBookmarks.getObject(%index).getTransformF();
    Game.camera.setTransformF(%trans);
}
...
function EditorGui::addCameraBookmarkByGui(%this) {
    ...
    for(%i = 0; ; %i++){
        %name = "NewCamera_" @ %i;
        if (!CameraBookmarks.findObjectByInternalName(%name)) break;
    }
...
//note: syncCameraGui is also called by C++ side right after setCameraHandler
function EditorGui::syncCameraGui(%this) {
    ...
    if (%displayType != $EditTSCtrl::DisplayTypePerspective) {
        ...
        Game.camera.controlMode = "Fly";
        Game.camera.setRotation(%camRot);
    ...
    %flyModeRadioItem = -1;
    if (Game.getCameraHandler() != getPlayerVehicle(0)) {
        %mode = Game.camera.getMode();
        if (%mode $= "Fly" && Game.camera.newtonMode && Game.camera.mass != 1) {
            if (Game.camera.newtonRotation == true) {
        ...
        EditorGuiStatusBar.setCamera("Game Camera");
    }
}
...
function WorldEditorPlugin::onActivated(%this) {
    ...
    if (EWorldEditor.ETransformSelectionDisplayed) ETransformSelection.setVisible(true);
...
function WorldEditorInspectorPlugin::handleDelete(%this) {
    // The tree handles deletion and notifies the world editor to clear its selection.
    // This is because non-SceneObject elements like SimGroups also need to be destroyed.
    // See EditorTree::onObjectDeleteCompleted().
    %selSize = EWorldEditor.getSelectionSize();
    if (%selSize > 0) EditorTree.deleteSelection();
}

function WorldEditorInspectorPlugin::handleDeselect() { EWorldEditor.clearSelection(); }
function WorldEditorInspectorPlugin::handleCut()      { EWorldEditor.cutSelection(); }
function WorldEditorInspectorPlugin::handleCopy()     { EWorldEditor.copySelection(); }
function WorldEditorInspectorPlugin::handlePaste()    { EWorldEditor.pasteSelection(); }
...
function TerrainEditorPlugin::onWorldEditorStartup(%this) {
    ...
    %map.bindCmd(keyboard, "4", "ToolsPaletteArray->smoothHeightNew.performClick();", "");    // new Smooth
...
function TerrainEditorPlugin::validateBrushSize(%this) {
    ...
    if (%val < %minBrushSize) $ThisControl.setValue(%minBrushSize);
    else if (%val > %maxBrushSize) $ThisControl.setValue(%maxBrushSize);
}
...
function EWorldEditor::areAllSelectedObjectsOfType(%this, %className) {
    ...
    if (!isObject(%activeSelection)) return false;

    %count = %activeSelection.getCount();
    for(%i = 0; %i < %count; %i ++) {
        %obj = %activeSelection.getObject(%i);
        if (!%obj.isMemberOfClass(%className)) return false;
    }
...
function EditorTree::onDeleteObject(%this, %object) {
    ...
    // We're gonna delete this ourselves in the completion callback.
    return true;
}

function EditorTree::onObjectDeleteCompleted(%this) {
    // This can be called when a deletion is attempted but nothing was actually deleted (cannot delete the root of the tree) so only submit the undo if we really deleted something.
    if (%this.undoDeleteList !$= "") MEDeleteUndoAction::submit(%this.undoDeleteList);

    // Let the world editor know to clear its selection.
...
function EditorTree::onAddSelection(%this, %obj, %isLastSelection, %isFirstSelection) {
    EWorldEditor.selectObject(%obj, %isLastSelection, %isFirstSelection);
    %selSize = EWorldEditor.getSelectionSize();
    %lockCount = EWorldEditor.getSelectionLockCount();
    ...
    if (%selSize > 0 && %lockCount == 0) EWTreeWindow-->DeleteSelection.command = "EditorMenuEditDelete();";
    else EWTreeWindow-->DeleteSelection.command = "";

    if (%isLastSelection) Inspector.addInspect(%obj);
    else Inspector.addInspect(%obj, false);

}
function EditorTree::onRemoveSelection(%this, %obj, %isLastSelection, %isFirstSelection) {
    EWorldEditor.unselectObject(%obj, %isLastSelection, %isFirstSelection);
    Inspector.removeInspect(%obj);
}
function EditorTree::onSelect(%this, %obj) {
}

function EditorTree::onUnselect(%this, %obj, %isLastSelection, %isFirstSelection) {
    EWorldEditor.unselectObject(%obj, %isLastSelection, %isFirstSelection);
}
...
//setdefault spawnpoint testing function
function setDefaultSP(%object1) {
    LuaExecuteQueueString("setSpawnpoint.setDefaultSP(\""@ %object1 @ "\")");
}
...
function EditorTree::onRightMouseUp(%this, %itemId, %mouse, %obj) {
    ...
    if (%this.getSelectedItemsCount() > 1) {
        ...
        // figure if thy are all the same type tht we need
        %count = %this.getSelectedItemsCount();
        %selected = %this.getSelectedItemList();
        %allVehicles = true;
        foreach$( %id in %selected ) {
            if(!EditorTree.getItemValue(%id).isMemberOfClass("BeamNGVehicle")) {
                %allVehicles = false;
                break;
            }
        }
        // okay, lets inject more menu items then
        if(%allVehicles) {
            %popup = ETMultiBeamNGVehiceContextPopup;
            if (!isObject(%popup)) {
                %popup = new PopupMenu(ETMultiBeamNGVehiceContextPopup : ETMultiSelectionContextPopup)
                {
                    superClass = "MenuBuilder";
                    isPopup = "1";
                    item[ 2 ] = "-";
                    item[ 3 ] = "Reset" TAB "" TAB "foreach$( %id in EditorTree.getSelectedItemList() ) EditorTree.getItemValue( %id ).reset();";
                    item[ 4 ] = "Reset + Pause" TAB "" TAB "foreach$( %id in EditorTree.getSelectedItemList() ) { EditorTree.getItemValue( %id ).reset(); EditorTree.getItemValue( %id ).pause(); }";
                    item[ 5 ] = "Reload" TAB "" TAB "foreach$( %id in EditorTree.getSelectedItemList() ) schedule(200, 0, EditorTree.getItemValue( %id ).reload());";
                    item[ 6 ] = "Reload (Autoplace)" TAB "" TAB "foreach$( %id in EditorTree.getSelectedItemList() ) schedule(200, 0, EditorTree.getItemValue( %id ).reloadAP());";
                };
            }
        }
    } else if (%obj.isMemberOfClass("CameraBookmark")) { // Open context menu if this is a CameraBookmark
    ...
    } else if (%obj.name $= "CameraBookmarks") { // Open context menu if this is set CameraBookmarks group.
    ...
    } else if (%obj.isMemberOfClass("Marker")) { // Open context menu if this is a Marker
        %popup = ETMarkerContextPopup;
        if (!isObject(%popup)) {
            %popup = new PopupMenu(ETMarkerContextPopup) {
                superClass = "MenuBuilder";
                isPopup = "1";
                item[ 0 ] = "Set camera at Marker" TAB "" TAB "%this.object.setCameraAtMarker();";
                item[ 1 ] = "Set Marker at Camera" TAB "" TAB "%this.object.setMarkerAtCamera();";
                object = -1;
            };
        }
    ...
    } else if (%obj.isMemberOfClass("SimGroup")) { // Open context menu if this is a SimGroup
        %popup = ETSimGroupContextPopup;
        if (!isObject(%popup)) {
            %popup = new PopupMenu(ETSimGroupContextPopup) {
                superClass = "MenuBuilder";
                isPopup = "1";
                item[ 0 ] = "Pack Prefab" TAB "" TAB "EWorldEditor.clearSelection();EWorldEditor.selectObject(%this.object); if (%this.object.prefab_filename !$= \"\") {EWorldEditor.makeSelectionPrefab(%this.object.prefab_filename, %this.object.prefab_name, %this.object.prefab_loadmode);} else {EditorMakePrefab();}";
                item[ 1 ] = "-";
                item[ 2 ] = "Rename" TAB "" TAB "EditorTree.showItemRenameCtrl(EditorTree.findItemByObjectId(%this.object));";
                item[ 3 ] = "Delete" TAB "" TAB "EWorldEditor.deleteMissionObject(%this.object);";
                item[ 4 ] = "Inspect" TAB "" TAB "inspectObject(%this.object);";
                item[ 5 ] = "-";
                item[ 6 ] = "Toggle Lock Children" TAB "" TAB "EWorldEditor.toggleLockChildren(%this.object);";
                item[ 7 ] = "Toggle Hide Children" TAB "" TAB "EWorldEditor.toggleHideChildren(%this.object);";
                item[ 8 ] = "-";
                item[ 9 ] = "Group" TAB "" TAB "EWorldEditor.addSimGroup(true);";
                item[ 10 ] = "-";
                item[ 11 ] = "Add New Objects Here" TAB "" TAB "EWCreatorWindow.setNewObjectGroup(%this.object);";
                item[ 12 ] = "Add Children to Selection" TAB "" TAB "EWorldEditor.selectAllObjectsInSet(%this.object, false);";
                item[ 13 ] = "Remove Children from Selection" TAB "" TAB "EWorldEditor.selectAllObjectsInSet(%this.object, true);";
        ...
        %popup.enableItem(0, %obj.unpacked_prefab $= "1");

        %haveObjectEntries = true;
        %haveLockAndHideEntries = false;
    } else { // Open generic context menu.
        ...
        if (!isObject(%popup)) {
            %popup = new PopupMenu(ETContextPopup) {
                ...
                //setDefaultSP default spawnpoint testing item
                item[ 8 ] = "Set default Spawnpoint" TAB "" TAB "setDefaultSP(%this.object.getName());";
                object = -1;
            };
        }
        %popup.object = %obj;
        %popup.enableItem(8, %obj.getClassName() $= "SpawnSphere");

        if (%obj.isMemberOfClass("ConvexShape")) { // Specialized version for ConvexShapes.
        ...
        if (%obj.isMemberOfClass("BeamNGVehicle")) { // Specialized version for BeamNGVehicle.
            %popup = ETBeamNGVehiceContextPopup;
            if (!isObject(%popup))
                %popup = new PopupMenu(ETBeamNGVehiceContextPopup : ETContextPopup)
                {
                    superClass = "MenuBuilder";
                    isPopup = "1";
                    item[ 8 ] = "-";
                    item[ 9 ] = "Pause toggle" TAB "" TAB "%this.object.togglepause();";
                    item[ 10 ] = "Reset" TAB "" TAB "%this.object.reset();";
                    item[ 11 ] = "Reset + Pause" TAB "" TAB "%this.object.reset(); %this.object.pause();";
                    item[ 12 ] = "Reload" TAB "" TAB "schedule(200, 0, %this.object.reload());";
                    item[ 13 ] = "Reload (Autoplace)" TAB "" TAB "schedule(200, 0, %this.object.reloadAP())";
                };

        } else if (%obj.isMemberOfClass("Prefab")) { // Specialized version for Prefab.
            %popup = ETPrefabContextPopup;
            if (!isObject(%popup))
                %popup = new PopupMenu(ETPrefabContextPopup : ETContextPopup)
                {
                    superClass = "MenuBuilder";
                    isPopup = "1";

                    item[ 8 ] = "-";
                    item[ 9 ] = "Unpack Prefab" TAB "" TAB "EWorldEditor.clearSelection();EWorldEditor.selectObject(%this.object);EWorldEditor.explodeSelectedPrefab();";
                    item[ 10 ] = "Load Prefab" TAB "" TAB "%this.object.load();";
                    item[ 11 ] = "Unload Prefab" TAB "" TAB "%this.object.unload();";
                };
            %loaded = %obj.isLoaded();
            //%popup.enableItem(9, %loaded == 0);
            %popup.enableItem(10, %loaded == 0);
            %popup.enableItem(11, %loaded == 1);
        } else if (%obj.isMemberOfClass("Zone") || %obj.isMemberOfClass("Portal") || %obj.isMemberOfClass("OcclusionVolume") || %obj.isMemberOfClass("SFXSpace")) { // Specialized version for polyhedral objects.
...
function EditorTree::isValidDragTarget(%this, %id, %obj) {
    if (%obj.isMemberOfClass("SimPath")) return EWorldEditor.areAllSelectedObjectsOfType("Marker");
    if (%obj.name $= "CameraBookmarks") return EWorldEditor.areAllSelectedObjectsOfType("CameraBookmark");
    else return (%obj.getClassName() $= "SimGroup");
}

function EditorTree::onBeginReparenting(%this) {
    if (isObject(%this.reparentUndoAction)) %this.reparentUndoAction.delete();
...
function EditorTree::onEndReparenting(%this) {
    ...
    if (%action.numObjects > 0) {
        if (%action.numObjects == 1) %action.actionName = "Reparent Object";
        else %action.actionName = "Reparent Objects";
...
function EditorTree::GetTooltipTSStatic   (%this, %obj) { return "Shape: " @ %obj.shapeName; }
function EditorTree::GetTooltipShapeBase  (%this, %obj) { return "Datablock: " @ %obj.dataBlock; }
function EditorTree::GetTooltipStaticShape(%this, %obj) { return "Datablock: " @ %obj.dataBlock; }
function EditorTree::GetTooltipItem       (%this, %obj) { return "Datablock: " @ %obj.dataBlock; }
function EditorTree::GetTooltipRigidShape (%this, %obj) { return "Datablock: " @ %obj.dataBlock; }
function EditorTree::GetTooltipPrefab     (%this, %obj) { return "File: " @ %obj.filename; }
...
function EditorTree::GetTooltipSFXEmitter(%this, %obj) {
    if (%obj.fileName $= "") return "Track: " @ %obj.track;
    else return "File: " @ %obj.fileName;
}
...
function EditorTree::GetTooltipWorldEditorSelection(%this, %obj) {
    ...
    if (!%obj.getCanSave()) %text = %text NL "Persistent: No";
    else %text = %text NL "Persistent: Yes";
    return %text;
}
...
function Editor::open(%this) {
    ...
    if (Canvas.getContent() == EditorGui.getId()) return;
    if (!EditorGui.isInitialized) EditorGui.init();
...
function Editor::close(%this, %gui) {
    ...
    if (isObject(MessageHud)) MessageHud.close();
...
function EditorLightingComplete() {
    ...
    if ($RelightCallback !$= "") eval($RelightCallback);
    $RelightCallback = "";
}

function updateEditorLightingProgress() {
    RelightProgress.setValue(($SceneLighting::lightingProgress));
    if ($lightingMission) $lightingProgressThread = schedule(1, 0, "updateEditorLightingProgress");
}

function Editor::lightScene(%this, %callback, %forceAlways) {
    if ($lightingMission) return;
...
function EditorGui::handleEscape(%this) {
    %result = false;
    if (isObject(%this.currentEditor)) %result = %this.currentEditor.handleEscape();
    if (!%result) Editor.close("OnlyGui");
}

function EditTSCtrl::updateGizmoMode(%this, %mode) {
    // Called when the gizmo mode is changed from C++
    if (%mode $= "None") EditorGuiToolbar->NoneModeBtn.performClick();
    else if (%mode $= "Move") EditorGuiToolbar->MoveModeBtn.performClick();
    else if (%mode $= "Rotate") EditorGuiToolbar->RotateModeBtn.performClick();
    else if (%mode $= "Scale") EditorGuiToolbar->ScaleModeBtn.performClick();
}
...
function EWorldEditor::syncToolPalette(%this) {
    switch$ (GlobalGizmoProfile.mode) {
        case "None": EWorldEditorNoneModeBtn.performClick();
        case "Move": EWorldEditorMoveModeBtn.performClick();
        case "Rotate": EWorldEditorRotateModeBtn.performClick();
        case "Scale": EWorldEditorScaleModeBtn.performClick();
    }
}
...
function EWorldEditor::addSimGroup(%this, %groupCurrentSelection) {
    ...
    // If we are about to do a group-selected as well, starting recording an undo compound.
    if (%groupCurrentSelection) Editor.getUndoManager().pushCompound("Group Selected");
    ...
    if (%groupCurrentSelection) Editor.getUndoManager().popCompound();

    // When not grouping selection, make the newly created SimGroup the current selection.
    if (!%groupCurrentSelection) {
        EWorldEditor.clearSelection();
        EWorldEditor.selectObject(%object);
    }

    %this.syncGui(); // Refresh the Gui.
}

function EWorldEditor::toggleLockChildren(%this, %simGroup) {
    foreach (%child in %simGroup) {
        if (%child.isMemberOfClass("SimGroup")) %this.toggleLockChildren(%child);
        else %child.setLocked(!%child.locked);
    }
    EWorldEditor.syncGui();
}
...
function EWorldEditor::toggleHideChildren(%this, %simGroup) {
    foreach (%child in %simGroup) {
        if (%child.isMemberOfClass("SimGroup")) %this.toggleHideChildren(%child);
        else %this.hideObject(%child, !%child.hidden);
    }
    EWorldEditor.syncGui();
}
...
function EWorldEditor::selectAllObjectsInSet(%this, %set, %deselect) {
    ...
    foreach (%obj in %set) {
        if (%deselect) %this.unselectObject(%obj);
...
function objectSnapDropdownDecoy::onMouseLeave() { objectSnapDropdown.toggle(); }
function objectCenterDropdownDecoy::onMouseLeave() { objectCenterDropdown.toggle(); }
function objectTransformDropdownDecoy::onMouseLeave() { objectTransformDropdown.toggle(); }

function EWAddSimGroupButton::onDefaultClick(%this) { EWorldEditor.addSimGroup(); }
function EWAddSimGroupButton::onCtrlClick(%this) { EWorldEditor.addSimGroup(true); }

function EWToolsToolbar::reset(%this) {
    %count = ToolsToolbarArray.getCount();
    for(%i = 0 ; %i < %count; %i++) ToolsToolbarArray.getObject(%i).setVisible(true);
...
function EWToolsToolbar::toggleSize(%this, %useDynamics) {
    // toggles the size of the tooltoolbar. also goes through and hides each control not currently selected. we hide the controls in a very neat, spiffy way
    if (%this.isClosed == 0) {
...
function EWToolsToolbarDecoy::onMouseEnter(%this) { EWToolsToolbar.toggleSize(true); }
function EWToolsToolbarDecoy::onMouseLeave(%this) { EWToolsToolbar.toggleSize(true); }

function EditorGuiStatusBar::reset(%this)               { EWorldEditorStatusBarInfo.clearInfo(); }
function EditorGuiStatusBar::getInfo(%this)             { return EWorldEditorStatusBarInfo.getValue(); }
function EditorGuiStatusBar::setInfo(%this, %text)      { EWorldEditorStatusBarInfo.setText(%text); }
function EditorGuiStatusBar::clearInfo(%this)           { EWorldEditorStatusBarInfo.setText(""); }
function EditorGuiStatusBar::getSelection(%this)        { return EWorldEditorStatusBarSelection.getValue(); }
function EditorGuiStatusBar::setSelection(%this, %text) { EWorldEditorStatusBarSelection.setText(%text); }

function EditorGuiStatusBar::setSelectionObjectsByCount(%this, %count) {
    %text = " objects selected";
    if (%count == 1) %text = " object selected";
...
function EWorldEditorStatusBarCamera::onSelect(%this, %id, %text) {
    switch$(%text) {
        case "Top View":
            LuaExecuteQueueString("commands.setEditorCameraStandard()");
        ...
        case "Game Camera":
            LuaExecuteQueueString("commands.setGameCamera()");
            EditorGui.setDisplayType($EditTsCtrl::DisplayTypePerspective);
        case "Orbit Camera":
            LuaExecuteQueueString("commands.setEditorOrbitCamera()");
            EditorGui.setDisplayType($EditTsCtrl::DisplayTypePerspective);
        case "Smooth Camera":
            LuaExecuteQueueString("commands.setEditorCameraNewton()");
            EditorGui.setDisplayType($EditTsCtrl::DisplayTypePerspective);
        case "Smooth Rot Camera":
            LuaExecuteQueueString("commands.setEditorCameraNewtonDamped()");
            EditorGui.setDisplayType($EditTsCtrl::DisplayTypePerspective);
    }
}

// Each a gui slider bar is pushed on the editor gui, it maps itself with value located in its connected text control
...
// Callbacks to close the dropdown slider controls like the camera speed, that are marked with this class name.
function EditorDropdownSliderContainer::onMouseDown(%this) { Canvas.popDialog(%this); }
function EditorDropdownSliderContainer::onRightMouseDown(%this) { Canvas.popDialog(%this); }
