    ...
    if o and o.drivability > 0 then
      ...
      if segCount > 0 then
        ...
        for i = 0, segCount do
          ...
          debugDrawer:drawSphere(n.pos:toPoint3F(), n.radius, ColorF(1,0,0, 0.1 * vis))
          --debugDrawer:drawText(pp, String(tostring(nid)), ColorF(0,0,0,1))
          if lastData then
            debugDrawer:drawSquarePrism(lastData.pos:toPoint3F(), n.pos:toPoint3F(), Point2F(0.6, lastData.radius*2), Point2F(0.6, n.radius*2), ColorF(0,0,1,0.1*vis))
            ...
            local function drawLane(ldir, lmid, lwidth, color)
              ...
              for o = 0, 1000 do -- failsafe for not running into endless loops
                ...
                data.position = posDetail:toPoint3F()
                data.color = color
                data.fadeStart = fadeStart
                data.fadeEnd = fadeEnd
                local s = lrad * 2 * 0.9
                data.scale = Point3F(s, s, s)
                local d = dir * ldir
                data.forwardVec = d:toPoint3F()
