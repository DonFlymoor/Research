...
local function onBusUpdate(state)
  ...
  -- data.position = state.pos:toPoint3F()
  -- data.color = ColorF(0.2, 0.53, 1, alpha )
  -- data.forwardVec = normal:toPoint3F()
  -- data.scale = Point3F(stepDistance, stepDistance, 1.5)
...
local function requestState()
  local tmp={}
  for _,v in pairs(busLines.routes) do
    table.insert(tmp, {txt=tostring(v.routeID).." "..tostring(v.direction), val=tostring(v.routeID).."|"..tostring(v.variance)})
  end

  guihooks.trigger('busRouteReceived', tmp)
