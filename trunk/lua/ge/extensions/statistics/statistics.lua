...
local function enableStatisticTracking(vehicleID, statisticName)
  ...
  for _, instance in pairs(statEntry.instances) do
    ...
    if key == getStatKey('altitude') then
       statEntry.initialZPos = vec3(vehicle:getPosition()).z
    elseif key == getStatKey('distance') then
      statEntry.initialPos = vec3(vehicle:getPosition())
    end
  end
end
  ...
  if #altitudeProperties > 0 then
    for i,entry in ipairs(altitudeProperties) do
      ...
      if entry.target == vehicleName then
        ...
        local altitudeData = {label='Altitude',  initialZPos=0, value=0.0, decimals=2, enabled = true, source=altitudeSource}
  ...
  if #damageProperties > 0 then
    for i,entry in ipairs(damageProperties) do
      ...
      if entry.target == vehicleName then
        ...
        local damageData = {label='Damage', value=0.0, decimals=1, predefinedUnit="%", enabled = true, source=damageSource}
  ...
  if #distanceProperties > 0 then
    for i,entry in ipairs(distanceProperties) do
      ...
      if entry.target == vehicleName then
        ...
        local distanceData = {label='Distance', initialPos=vec3(0,0,0), value=0.0, decimals=2, enabled=true, source=distanceSource}
  ...
  if #speedProperties > 0 then
    for i,entry in ipairs(speedProperties) do
      ...
      if entry.target == vehicleName then
        ...
        local speedData = {label='Max Speed', value=0.0, decimals=2, unit='speed', enabled = true, source=speedSource}
  ...
  if #timeProperties > 0 then
    for i,entry in ipairs(timeProperties) do
      if entry.target == vehicleName then
        ...
        local timeData = {label='Time', value=0.0, predefinedUnit = 's', decimals=2, enabled = true, source=timeSource}
  ...
  for vehicleID, vehicleTable in pairs(statsTable) do
    ...
    if data and data.enabled then
      for _,instance in pairs(data.instances) do
        ...
        if vehicle then
          local pos = vec3(vehicle:getPosition())
