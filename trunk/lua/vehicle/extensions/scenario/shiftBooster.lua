...
local function applyBoost(strength)
    ...
    local forceVec = vec3(obj:getForwardVector())
    -- get a boost vector
    forceVec:normalize()
...
local function onDebugDraw(focusPos)
    if boostTime < 0.01 then return end
    obj.debugDrawProxy:drawSphere(boostTime, obj:getPosition() + float3(0,0,2), color(math.sin(boostTime) * 255, math.cos(boostTime * 2) * 255,0,255))
end
