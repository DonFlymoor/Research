...
local function setTwoStep(enabled)
  if engine then
    ...
    gui.message(string.format("Two-Step is %s (%d RPM)", (enabled and "enabled" or "disabled"), tempRevLimiterRPM), 2, "vehicle.twoStep.status")
  end
end
...
local function changeTwoStepRPM(amount)
  tempRevLimiterRPM = tempRevLimiterRPM + amount
  tempRevLimiterAV = tempRevLimiterRPM * rpmToAV
  gui.message("Two-Step launch RPM: " .. tostring(tempRevLimiterRPM), 3, "vehicle.twoStep.rpm")
end

local function updateGFX(dt)
  if twoStepState == "idle" then
    if electrics.values.wheelspeed <= 0.5 and electrics.values.brake >= 0.8 and electrics.values.throttle >= 1 then
      twoStepState = "armed"
    end
  elseif twoStepState == "armed" then
    engine:setTempRevLimiter(tempRevLimiterAV)

    if electrics.values.wheelspeed > 0.5 and electrics.values.brake <= 0 then
      ...
      core_quickAccess.addEntry(
        {
          ...
          generator = function(entries)
            table.insert(entries, {title = "Launch Control", priority = 40, ["goto"] = "/powertrain/twoStep/", icon = "radial_flee"})
          end
        }
      )
