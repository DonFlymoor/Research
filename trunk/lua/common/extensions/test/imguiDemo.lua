...
local imguiCol_Count = tonumber(im.ImGuiCol("ImGuiCol_COUNT"))
-- Add Rect Filled values
local vec2B = im.ImVec2(200, 20)
-- PlotLines example values
local plot_lines_window_open = im.BoolPtr(false)
local imgui_windowFlag_MenuBar = im.ImGuiWindowFlags("ImGuiWindowFlags_MenuBar")
...
local nextWindowPosCond = im.ImGuiCond("ImGuiCond_FirstUseEver")
local nextWindowSize = im.ImVec2(500, 440)
local nextWindowSizeCond = im.ImGuiCond("ImGuiCond_FirstUseEver")
...
local function ShowExampleMenuFile()
  ...
  if im.BeginMenu("Colors", im.BoolTrue) then
    ...
    for i = 0, tonumber(im.ImGuiCol("ImGuiCol_COUNT")) do
...
local function ShowExampleAppLayout()
  im.SetNextWindowSize(nextWindowSize, im.ImGuiCond("ImGuiCond_FirstUseEver"))
  if im.Begin("Example: Layout", appLayoutWindowOpen, im.ImGuiWindowFlags("ImGuiWindowFlags_MenuBar")) then
    -- MenuBar
...
local function ShowUserGuide()
  ...
  if im.GetImGuiIO_FontAllowUserScaling() == true then
...
local function Widgets_Basic()
  if im.TreeNode1("Basic") then
    ...
    for i = 0, 6 do
      ...
      im.PushID4(i)
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_Button"), im.ColorConvertHSVtoRGB( i/7.0, 0.6, 0.6, 1.0))
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_ButtonHovered"), im.ColorConvertHSVtoRGB( i/7.0, 0.7, 0.7))
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_ButtonActive"), im.ColorConvertHSVtoRGB( i/7.0, 0.8, 0.8))
      im.Button("Click")
    ...
    if im.ArrowButton("##left", im.ImGuiDir("ImGuiDir_Left")) then end
    im.SameLine(0.0, spacing[1].x)
    if im.ArrowButton("##left", im.ImGuiDir("ImGuiDir_Right")) then end
...
local function Widgets_Trees()
  if im.TreeNode1("Trees") then
    ...
    if im.TreeNode1("Advanced, with Selectable nodes") then
      ...
      im.PushStyleVar1(im.ImGuiStyleVar("ImGuiStyleVar_IndentSpacing"), im.GetFontSize()*3); -- Increase spacing to differentiate leaves from expanded contents.
      for i = 0, 5 do
        -- Disable the default open on single-click behavior and pass in Selected flag according to our selection state.
        -- ImGuiTreeNodeFlags node_flags = ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_OpenOnDoubleClick | ((selection_mask & (1 << i)) ? ImGuiTreeNodeFlags_Selected : 0)
        local node_flags = bor(im.ImGuiTreeNodeFlags("ImGuiTreeNodeFlags_OpenOnArrow"), im.ImGuiTreeNodeFlags("ImGuiTreeNodeFlags_OpenOnDoubleClick"), (band(selection_mask, blshift(1, i)) > 0) and im.ImGuiTreeNodeFlags("ImGuiTreeNodeFlags_Selected") or 0)
        if i < 3 then
          -- Node
          ...
          node_flags = node_flags or im.ImGuiTreeNodeFlags("ImGuiTreeNodeFlags_Leaf") -- | ImGuiTreeNodeFlags_Bullet
      ...
      if var.node_clicked == -1 then
        ...
        local imGuiIO = ffi.new("ImGuiIO[1]")
        if im.GetIO(imGuiIO)[1].KeyCtrl then
...
local function Widgets_Text()
  if im.TreeNode1("Text") then
    ...
    if im.TreeNode1("Word Wrapping") then
      ...
      im.GetCursorScreenPos(var.pos)
      im.ImDrawList_AddRectFilled(im.GetWindowDrawList(), im.ImVec2(var.pos[0].x + var.wrap_width[0], var.pos[0].y), im.ImVec2(var.pos[0].x + var.wrap_width[0] + 10, var.pos[0].y + im.GetTextLineHeight()), im.GetColorU322( im.ImVec4(1, 0, 1, 1)))
      ...
      im.GetItemRectMin(var.itemRectMin)
      im.GetItemRectMax(var.itemRectMax)
      im.ImDrawList_AddRect(im.GetWindowDrawList(), var.itemRectMin, var.itemRectMax, im.GetColorU322( im.ImVec4(1, 1, 0, 1)))
      im.PopTextWrapPos()
...
local function Widgets_Combo()
  if im.TreeNode1("Combo") then
    ...
    im.CheckboxFlags("ImGuiComboFlags_PopupAlignLeft", var.flags, im.ImGuiComboFlags("ImGuiComboFlags_PopupAlignLeft"))
    if im.CheckboxFlags("ImGuiComboFlags_NoArrowButton", var.flags, im.ImGuiComboFlags("ImGuiComboFlags_NoArrowButton")) then
      -- flags &=   ~ImGuiComboFlags_NoPreview;     // Clear the other flag, as we cannot combine both
      var.flags[0] = bit.band(var.flags[1], bit.bnot(bit.tobit(im.ImGuiComboFlags("ImGuiComboFlags_NoPreview")))) -- Clear the other flag, as we cannot combine both
    end
    if im.CheckboxFlags("ImGuiComboFlags_NoPreview", var.flags, im.ImGuiComboFlags("ImGuiComboFlags_NoPreview")) then
      -- flags &= ~ImGuiComboFlags_NoArrowButton; // Clear the other flag, as we cannot combine both
      var.flags[0] = bit.band(tonumber(var.flags[1]), bit.bnot(bit.tobit(im.ImGuiComboFlags("ImGuiComboFlags_NoArrowButton")))) -- Clear the other flag, as we cannot combine both
    end
  ...
  if im.TreeNode1("Selectables") then
    ...
    if im.TreeNode1("Basic") then
      im.Selectable1("1. I am selectable", var.selection[0])
        ...
        if im.Selectable1("5. I am double clickable", var.selection[4], im.ImGuiSelectableFlags("ImGuiSelectableFlags_AllowDoubleClick")) then
    ...
    if im.TreeNode1("Selection State: Multiple Selection") then
      ...
      for n = 0, 4 do
        ...
        if im.Selectable1(buf, var.selection_2[n]) then
          if not im.ImGuiIO_KeyCtrl() then -- Clear selection when CTRL is not held
...
local function Widgets_FilteredTextInput()
  if im.TreeNode1("Filtered Text Input") then
    ...
    im.InputText("decimal", var.buf2, 64, im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsDecimal"))
    im.InputText("hexadecimal", var.buf3, 64, bit.bor(im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsHexadecimal"), im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsUppercase") ))
    im.InputText("uppercase", var.buf4, 64, im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsUppercase"))
    im.InputText("no blank", var.buf5, 64, im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsNoBlank"))
    ...
    im.InputText("password", var.bufpass, 64, bit.bor(im.ImGuiInputTextFlags("ImGuiInputTextFlags_Password"), im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsNoBlank") ))
    im.SameLine()
    im.ShowHelpMarker("Display all characters as '*'.\nDisable clipboard cut and copy.\nDisable logging.\n")
    im.InputText("password (clear)", var.bufpass, 64, im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsNoBlank"))
...
local function Widgets_MultiLineTextInput()
  if im.TreeNode1("Multi-line Text Input") then
    im.PushStyleVar2(im.ImGuiStyleVar("ImGuiStyleVar_FramePadding"), im.ImVec2(0,0))
    im.Checkbox("Read-only", var.read_only)
    im.PopStyleVar()
    local flag
    if var.read_only[0] then flag = im.ImGuiInputTextFlags("ImGuiInputTextFlags_ReadOnly") else flag = 0 end
    im.InputTextMultiline("##source", var.text, im.GetLengthArrayCharPtr(var.text), im.ImVec2(-1.0, im.GetTextLineHeight() * 16), bit.bor(im.ImGuiInputTextFlags("ImGuiInputTextFlags_AllowTabInput"), flag))
    im.TreePop()
  end
end
...
local function Widgets_ColorPicker()
  if im.TreeNode1("Color/Picker Widgets") then
    ...
    local misc_flags = bor(
      colorPicker.hdr[0] and im.ImGuiColorEditFlags("ImGuiColorEditFlags_HDR") or 0,
      colorPicker.alpha_half_preview[0] and im.ImGuiColorEditFlags("ImGuiColorEditFlags_AlphaPreviewHalf") or (colorPicker.alpha_preview[0] and im.ImGuiColorEditFlags("ImGuiColorEditFlags_AlphaPreview") or 0),
      (colorPicker.options_menu[0] and 0 or im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoOptions"))
    )
    ...
    im.ColorEdit4("MyColor##2", colorPicker.color2, bor(im.ImGuiColorEditFlags("ImGuiColorEditFlags_HSV"), misc_flags))

    im.Text("Color widget with Float Display:")
    im.ColorEdit4("MyColor##2f", colorPicker.color2, bor(im.ImGuiColorEditFlags("ImGuiColorEditFlags_Float"), misc_flags))
    ...
    im.ColorEdit4("MyColor##3", colorPicker.color2, bor(im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoInputs"), im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoLabel"), misc_flags))
    ...
    if im.BeginPopup("mypicker") then
      ...
      im.Separator()
      im.ColorPicker4("##picker", colorPicker.color2, bor(misc_flags, im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoSidePreview"), im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoSmallPreview")))
      im.SameLine()
      im.BeginGroup()
      im.Text("Current")
      im.ColorButton("##current", colorPicker.color, bor(im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoPicker"), im.ImGuiColorEditFlags("ImGuiColorEditFlags_AlphaPreviewHalf")), im.ImVec2(60,40))
      im.Text("Previous")
      if im.ColorButton("##previous", colorPicker.backup_color, bor(im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoPicker"), im.ImGuiColorEditFlags("ImGuiColorEditFlags_AlphaPreviewHalf")), im.ImVec2(60,40)) then
      ...
      for n = 0, im.GetLengthArrayImVec4(colorPicker.saved_palette) - 1 do
        ...
        if (n % 8) ~= 0 then
          ...
          im.ImGuiStyle_ItemSpacing(vec2)
          im.SameLine(0.0, vec2[0].y)
        end
        if im.ColorButton("##palette", colorPicker.saved_palette[n], bor(im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoAlpha"), im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoPicker"), im.ImGuiColorEditFlags("ImGuiColorEditFlags_NoTooltip")), im.ImVec2(20,20)) then
...
local function Widgets_DataTypes()
  if im.TreeNode1("Data Types") then
    ...
    im.DragScalar("drag s32",       im.ImGuiDataType("ImGuiDataType_S32"),    dataTypes.s32_v, dataTypes.drag_speed,  dataTypes.drag_clamp[0] and dataTypes.s32_zero or nil,  dataTypes.drag_clamp[0] and dataTypes.s32_fifty or nil)
    im.DragScalar("drag u32",       im.ImGuiDataType("ImGuiDataType_U32"),    dataTypes.u32_v, dataTypes.drag_speed,  dataTypes.drag_clamp[0] and dataTypes.u32_zero or nil,  dataTypes.drag_clamp[0] and dataTypes.u32_fifty or nil, "%u ms")
    im.DragScalar("drag s64",       im.ImGuiDataType("ImGuiDataType_S64"),    dataTypes.s64_v, dataTypes.drag_speed,  dataTypes.drag_clamp[0] and dataTypes.s64_zero or nil,  dataTypes.drag_clamp[0] and dataTypes.s64_fifty or nil)
    im.DragScalar("drag u64",       im.ImGuiDataType("ImGuiDataType_U64"),    dataTypes.u64_v, dataTypes.drag_speed,  dataTypes.drag_clamp[0] and dataTypes.u64_zero or nil,  dataTypes.drag_clamp[0] and dataTypes.u64_fifty or nil)
    im.DragScalar("drag float",     im.ImGuiDataType("ImGuiDataType_Float"),  dataTypes.f32_v, 0.005,                 dataTypes.f32_zero,                                     dataTypes.f32_one,                                      "%f",           1.0)
    im.DragScalar("drag float ^2",  im.ImGuiDataType("ImGuiDataType_Float"),  dataTypes.f32_v, 0.005,                 dataTypes.f32_zero,                                     dataTypes.f32_one,                                      "%f",           2.0)  im.SameLine() im.ShowHelpMarker("You can use the 'power' parameter to increase tweaking precision on one side of the range.")
    im.DragScalar("drag double",    im.ImGuiDataType("ImGuiDataType_Double"), dataTypes.f64_v, 0.0005,                dataTypes.f64_zero,                                     nil,                                                    "%.10f grams",  1.0)
    im.DragScalar("drag double ^2", im.ImGuiDataType("ImGuiDataType_Double"), dataTypes.f64_v, 0.0005,                dataTypes.f64_zero,                                     dataTypes.f64_one,                                      "0 < %.10f < 1", 2.0)

    im.Text("Sliders")
    im.SliderScalar("slider s32 low",     im.ImGuiDataType("ImGuiDataType_S32"),    dataTypes.s32_v, dataTypes.s32_zero, dataTypes.s32_fifty,"%d")
    im.SliderScalar("slider s32 high",    im.ImGuiDataType("ImGuiDataType_S32"),    dataTypes.s32_v, dataTypes.s32_hi_a, dataTypes.s32_hi_b, "%d")
    im.SliderScalar("slider s32 full",    im.ImGuiDataType("ImGuiDataType_S32"),    dataTypes.s32_v, dataTypes.s32_min,  dataTypes.s32_max,  "%d")
    im.SliderScalar("slider u32 low",     im.ImGuiDataType("ImGuiDataType_U32"),    dataTypes.u32_v, dataTypes.u32_zero, dataTypes.u32_fifty,"%u")
    im.SliderScalar("slider u32 high",    im.ImGuiDataType("ImGuiDataType_U32"),    dataTypes.u32_v, dataTypes.u32_hi_a, dataTypes.u32_hi_b, "%u")
    im.SliderScalar("slider u32 full",    im.ImGuiDataType("ImGuiDataType_U32"),    dataTypes.u32_v, dataTypes.u32_min,  dataTypes.u32_max,  "%u")
    im.SliderScalar("slider s64 low",     im.ImGuiDataType("ImGuiDataType_S64"),    dataTypes.s64_v, dataTypes.s64_zero, dataTypes.s64_fifty,"%I64d")
    im.SliderScalar("slider s64 high",    im.ImGuiDataType("ImGuiDataType_S64"),    dataTypes.s64_v, dataTypes.s64_hi_a, dataTypes.s64_hi_b, "%I64d")
    im.SliderScalar("slider s64 full",    im.ImGuiDataType("ImGuiDataType_S64"),    dataTypes.s64_v, dataTypes.s64_min,  dataTypes.s64_max,  "%I64d")
    im.SliderScalar("slider u64 low",     im.ImGuiDataType("ImGuiDataType_U64"),    dataTypes.u64_v, dataTypes.u64_zero, dataTypes.u64_fifty,"%I64u ms")
    im.SliderScalar("slider u64 high",    im.ImGuiDataType("ImGuiDataType_U64"),    dataTypes.u64_v, dataTypes.u64_hi_a, dataTypes.u64_hi_b, "%I64u ms")
    im.SliderScalar("slider u64 full",    im.ImGuiDataType("ImGuiDataType_U64"),    dataTypes.u64_v, dataTypes.u64_min,  dataTypes.u64_max,  "%I64u ms")
    im.SliderScalar("slider float low",   im.ImGuiDataType("ImGuiDataType_Float"),  dataTypes.f32_v, dataTypes.f32_zero, dataTypes.f32_one)
    im.SliderScalar("slider float low^2", im.ImGuiDataType("ImGuiDataType_Float"),  dataTypes.f32_v, dataTypes.f32_zero, dataTypes.f32_one,  "%.10f", 2.0)
    im.SliderScalar("slider float high",  im.ImGuiDataType("ImGuiDataType_Float"),  dataTypes.f32_v, dataTypes.f32_lo_a, dataTypes.f32_hi_a, "%e")
    im.SliderScalar("slider double low",  im.ImGuiDataType("ImGuiDataType_Double"), dataTypes.f64_v, dataTypes.f64_zero, dataTypes.f64_one,  "%.10f grams", 1.0)
    im.SliderScalar("slider double low^2",im.ImGuiDataType("ImGuiDataType_Double"), dataTypes.f64_v, dataTypes.f64_zero, dataTypes.f64_one,  "%.10f", 2.0)
    im.SliderScalar("slider double high", im.ImGuiDataType("ImGuiDataType_Double"), dataTypes.f64_v, dataTypes.f64_lo_a, dataTypes.f64_hi_a, "%e grams", 1.0)


    im.Text("Inputs")
    im.Checkbox("Show step buttons", dataTypes.inputs_step)
    im.InputScalar("input s32",     im.ImGuiDataType("ImGuiDataType_S32"),    dataTypes.s32_v, dataTypes.inputs_step and dataTypes.s32_one or nil, nil, "%d")
    im.InputScalar("input s32 hex", im.ImGuiDataType("ImGuiDataType_S32"),    dataTypes.s32_v, dataTypes.inputs_step and dataTypes.s32_one or nil, nil, "%08X", im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsHexadecimal"))
    im.InputScalar("input u32",     im.ImGuiDataType("ImGuiDataType_U32"),    dataTypes.u32_v, dataTypes.inputs_step and dataTypes.u32_one or nil, nil, "%u")
    im.InputScalar("input u32 hex", im.ImGuiDataType("ImGuiDataType_U32"),    dataTypes.u32_v, dataTypes.inputs_step and dataTypes.u32_one or nil, nil, "%08X", im.ImGuiInputTextFlags("ImGuiInputTextFlags_CharsHexadecimal"))
    im.InputScalar("input s64",     im.ImGuiDataType("ImGuiDataType_S64"),    dataTypes.s64_v, dataTypes.inputs_step and dataTypes.s64_one or nil)
    im.InputScalar("input u64",     im.ImGuiDataType("ImGuiDataType_U64"),    dataTypes.u64_v, dataTypes.inputs_step and dataTypes.u64_one or nil)
    im.InputScalar("input float",   im.ImGuiDataType("ImGuiDataType_Float"),  dataTypes.f32_v, dataTypes.inputs_step and dataTypes.f32_one or nil)
    im.InputScalar("input double",  im.ImGuiDataType("ImGuiDataType_Double"), dataTypes.f64_v, dataTypes.inputs_step and dataTypes.f64_one or nil)
...
local function Widgets_VerticalSlider()
  if im.TreeNode1("Vertical Sliders") then
    ...
    im.PushStyleVar2(im.ImGuiStyleVar("ImGuiStyleVar_ItemSpacing"), im.ImVec2(vertSlider.spacing, vertSlider.spacing))
    ...
    for i = 0, 6 do
      ...
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_FrameBg"), col)
      col = im.ColorConvertHSVtoRGB(i/7.0, 0.6, 0.5)
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_FrameBgHovered"), col)
      col = im.ColorConvertHSVtoRGB(i/7.0, 0.7, 0.5)
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_FrameBgActive"), col)
      col = im.ColorConvertHSVtoRGB(i/7.0, 0.9, 0.9)
      im.PushStyleColor2(im.ImGuiCol("ImGuiCol_SliderGrab"), col)
    ...
    for i = 0, 3 do
      if i > 0 then im.SameLine() end
      im.PushID4(i)
      im.PushStyleVar1(im.ImGuiStyleVar("ImGuiStyleVar_GrabMinSize"), 40)
...
local function Widgets_Focused()
  ...
  -- (because BulletText is an item itself and that would affect the output of IsItemHovered() we pass all state in a single call to simplify the code).
    ...
    im.BulletText(
      ...
      im.IsItemFocused(),
      im.IsItemHovered(),
      im.IsItemHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_AllowWhenBlockedByPopup")),
      im.IsItemHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_AllowWhenBlockedByActiveItem")),
      im.IsItemHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_AllowWhenOverlapped")),
      im.IsItemHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_RectOnly")),
      im.IsItemActive(),
    ...
    im.BulletText(
      ...
      im.IsWindowFocused(),
      im.IsWindowFocused(im.ImGuiFocusedFlags("ImGuiFocusedFlags_ChildWindows")),
      im.IsWindowFocused(bor(im.ImGuiFocusedFlags("ImGuiFocusedFlags_ChildWindows"), im.ImGuiFocusedFlags("ImGuiFocusedFlags_RootWindow"))),
      im.IsWindowFocused(im.ImGuiFocusedFlags("ImGuiFocusedFlags_RootWindow")),
      im.IsWindowFocused(im.ImGuiFocusedFlags("ImGuiFocusedFlags_AnyWindow"))
    )
    ...
    im.BulletText(
        ...
        im.IsWindowHovered(),
        im.IsWindowHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_AllowWhenBlockedByPopup")),
        im.IsWindowHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_AllowWhenBlockedByActiveItem")),
        im.IsWindowHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_ChildWindows")),
        im.IsWindowHovered(bor(im.ImGuiHoveredFlags("ImGuiHoveredFlags_ChildWindows"), im.ImGuiHoveredFlags("ImGuiHoveredFlags_RootWindow"))),
        im.IsWindowHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_RootWindow")),
        im.IsWindowHovered(im.ImGuiHoveredFlags("ImGuiHoveredFlags_AnyWindow"))
    )
...
local function Layout_ChildRegions()
  if im.TreeNode1("Child regions") then
    ...
    im.SameLine()
    im.PushItemWidth(100)
    goto_line = (bor((goto_line) and 1 or 0, (im.InputInt("##Line", childRegions.line, 0, 0, im.ImGuiInputTextFlags("ImGuiInputTextFlags_EnterReturnsTrue"))) and 1 or 0) == 1) and true or false
    im.PopItemWidth()

    -- Child 1: no border, enable horizontal scrollbar
    im.BeginChild1("Child1", im.ImVec2(im.GetWindowContentRegionWidth() * 0.5, 300), false, bor(im.ImGuiWindowFlags("ImGuiWindowFlags_HorizontalScrollbar"), (childRegions.disable_mouse_wheel[0] and im.ImGuiWindowFlags("ImGuiWindowFlags_NoScrollWithMouse") or 0)))
    for i = 0, 99 do
    ...
    im.PushStyleVar1(im.ImGuiStyleVar("ImGuiStyleVar_ChildRounding"), 5.0)
    im.BeginChild1("Child2", im.ImVec2(0,300), true, bor( (childRegions.disable_mouse_wheel[0] and im.ImGuiWindowFlags("ImGuiWindowFlags_NoScrollWithMouse") or 0) , (childRegions.disable_menu[0] and 0 or im.ImGuiWindowFlags("ImGuiWindowFlags_MenuBar")) ) )
...
local function Layout_Groups()
  if im.TreeNode1("Groups") then
        ...
        local size = im.ImVec2Ptr()
        im.GetItemRectSize(size)

        im.PlotHistogram1("##values", groups.values, groups.valuesSize, 0, nil, 0.0, 1.0, size[0])

        im.GetStyle(groups.style)
        im.Button("ACTION", im.ImVec2((size[0].x - groups.style[0].ItemSpacing.x) * 0.5,size[0].y))
        im.SameLine()
        im.Button("REACTION", im.ImVec2((size[0].x - groups.style[0].ItemSpacing.x) * 0.5,size[0].y))
        im.EndGroup()
        im.SameLine()
...
im.ImGuiStyle_ItemSpacing(textAlign.spacing)
...
scrolling.cursorStartPos = im.ImVec2Ptr(0,0)
  ...
  if im.TreeNode1("Scrolling") then
    ...
    for i = 0, 4 do
        ...
        if scrolling.scroll_to then
          im.GetCursorStartPos(scrolling.cursorStartPos)
          im.SetScrollFromPosY(scrolling.cursorStartPos[0].y + im.Float(scrolling.scroll_to_px[0]), im.Float(i * 0.25))
        end
        for line = 0, 99 do
...
horScroll.io = ffi.new('ImGuiIO[1]')
...
local function Layout_HorizontalScrolling()
  if im.TreeNode1("Horizontal Scrolling") then
    ...
    im.PushStyleVar1(im.ImGuiStyleVar("ImGuiStyleVar_FrameRounding"), 3.0)
    im.PushStyleVar2(im.ImGuiStyleVar("ImGuiStyleVar_FramePadding"), im.ImVec2(2.0, 1.0))
    im.BeginChild1("scrolling", im.ImVec2(0, im.GetFrameHeightWithSpacing()*7 + 30), true, im.ImGuiWindowFlags("ImGuiWindowFlags_HorizontalScrollbar"))
    for line = 0, horScroll.lines[0] - 1 do
      ...
      for n = 0, num_buttons-1 do
        ...
        local hue = n*0.05
        im.PushStyleColor2(im.ImGuiCol("ImGuiCol_Button"), im.ImVec4(hue, 0.6, 0.6, 1))
        im.PushStyleColor2(im.ImGuiCol("ImGuiCol_ButtonHovered"), im.ImVec4(hue, 0.7, 0.7, 1))
        im.PushStyleColor2(im.ImGuiCol("ImGuiCol_ButtonActive"), im.ImVec4(hue, 0.8, 0.8, 1))
    ...
    if im.IsItemActive() then
      im.GetIO(horScroll.io)
      horScroll.scroll_x_delta = -horScroll.io[0].DeltaTime * 1000.0
    end
    im.SameLine()
...
local function Layout_Clipping()
  if im.TreeNode1("Clipping") then
    ...
    if im.IsItemActive() and im.IsMouseDragging() then
      im.GetIO(clipping.io)
      clipping.offset[0].x = clipping.offset[0].x + clipping.io[0].MouseDelta.x
      clipping.offset[0].y = clipping.offset[0].y + clipping.io[0].MouseDelta.y
    end
...
local function Columns_Basic()
  ...
  if im.TreeNode1("Basic") then
    ...
    for i = 0, 2 do
      ...
      if im.Selectable1(label, columnsBasic.selected[0] == i, im.ImGuiSelectableFlags("ImGuiSelectableFlags_SpanAllColumns")) then columnsBasic.selected[0] = i end
...
local function Columns_HorizontalScrolling()
  if im.TreeNode1("Horizontal Scrolling") then
    ...
    im.BeginChild1("##ScrollingRegion", im.ImVec2(0, im.GetFontSize() * 20), false, im.ImGuiWindowFlags("ImGuiWindowFlags_HorizontalScrollbar"))
    im.Columns(10)
...
local function ImGuiDemo(p_open)
  ...
  if var.no_titlebar[0] then  var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_NoTitleBar")) end
  if var.no_scrollbar[0] then var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_NoScrollbar")) end
  if not var.no_menu[0] then  var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_MenuBar")) end
  if var.no_move[0] then      var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_NoMove")) end
  if var.no_resize[0] then    var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_NoResize")) end
  if var.no_collapse[0] then  var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_NoCollapse")) end
  if var.no_nav[0] then       var.imguiDemoWindowFlags = bit.bor(var.imguiDemoWindowFlags, im.ImGuiWindowFlags("ImGuiWindowFlags_NoNav")) end
  ...
  im.SetNextWindowSize(im.ImVec2(550,680), im.ImGuiCond("ImGuiCond_FirstUseEver"))
...
local function onUpdate()
  ...
  ImGuiDemo(var.imguiDemoWindowOpen)
...
local function onInit()
  log('I', logTag, "imguiDemo initialized!")
end
