...
singleton GFXStateBlockData( PFX_DOFFinalStateBlock )
{
    ...
    samplerStates[4] = SamplerClampPoint;
...
singleton PostEffect( DOFFinalPFX )
{
    ...
    texture[1] = "$inTex";
    texture[2] = "#largeBlur";
    texture[3] = "#prepass[RT0]";
    texture[4] = "#prepass[Depth]";
    target = "$backBuffer";
};
...
{
    if (!isObject(Game) || !isObject(Game.getCamera())) return;

    %mask = $TypeMasks::StaticObjectType | $TypeMasks::TerrainObjectType;
    %control = Game.getCamera();
