...
uniform_sampler1D( depthGradMap, 6);
uniform_sampler2D( prepassTex, 7);
...
uniform float4 debugColor0;
uniform float4 debugColor1;
#endif
...
float4 main( ConnectData IN ) : SV_Target
{
   ...
   float startDepth = decodeGBuffer( prepassDepthTex, prepassTex, prepassCoord, projParams ).w;  
   ...
   float prepassDepth = decodeGBuffer( prepassDepthTex, prepassTex, prepassCoord, projParams ).w;      
   ...
   else
   {
      ...
      if ( diff < 0 )
      {
         ...
         // Get prepass depth at the position of this distorted pixel.
         prepassDepth = decodeGBuffer( prepassDepthTex, prepassTex, prepassCoord, projParams ).w;
	 if ( prepassDepth > 0.99 )
            prepassDepth = 5.0;
   ...
   refMapColor = lerp( refMapColor, texCUBE( skyMap, reflectionVec )*float4(SPEC_COLOR, 1), NO_REFLECT );
...
#ifdef BGE_RENDER_ANNOTATION_BUILD
   OUT.rgb = (debugColor0.a > 0.1f) ? debugColor0.rgb : OUT.rgb;
   OUT.rgb = (debugColor0.a > 0.1f && factor < 0.02f) ? debugColor1.rgb : OUT.rgb;
#endif
