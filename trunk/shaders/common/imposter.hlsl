    ...
    */
    {
        ...
        index = int2( rotZ / equatorStepSize, polarIdx );
    }
    ...
    float4 uvSet = { uvs.zw * index, uvs.zw }; // pos and extend
    outTexCoord.x = uvSet.x + ( uvSet.z * (1 - sUVCornerExtent[corner].x) );
    outTexCoord.y = uvSet.y + ( uvSet.w * sUVCornerExtent[corner].y );

    // Needed for normal mapping and lighting.
    outWorldToTangent[0] = normalize(camRight); // T
    outWorldToTangent[1] = normalize(camUp); // B
    outWorldToTangent[2] = normalize(lookVec); // N
}
