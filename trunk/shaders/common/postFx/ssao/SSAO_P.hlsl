...
uniform_sampler2D( prepassMap, 0 );
uniform_sampler2D( prepassDepthMap, 1 );
uniform_sampler2D( randNormalTex, 2 );
uniform_sampler1D( powTable, 3);
                    ...
                    float normalDiff, float dt, float normalTol, float normalPow )
{
   ...
   return ( 1.0 - tex1Dlod( powTable, float4(delta, 0, 0, 0) ).r ) * normalDiff;
}


float4 main( PFXVertToPix IN ) : SV_TARGET
{          
static const float3 ptSphere[32] =
   {
   ...
   float4 noiseMapUV = float4( ( IN.uv2 * ( targetSize / texSize2 ) ).xy, 0, 0 );
   float3 reflectNormal = normalize( tex2Dlod( randNormalTex, noiseMapUV ).xyz * 2.0 - 1.0 );   
   //return float4( reflectNormal, 1 );
   
   float4 prepass = decodeGBuffer( prepassDepthMap, prepassMap, IN.uv0, projParams );
   ...
   for ( i = 0; i < sSampleCount; i++ )
   {
      ...
      occluderFragment = decodeGBuffer( prepassDepthMap, prepassMap, se.xy, projParams );                  
