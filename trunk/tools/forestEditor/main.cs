...
{
    //debug(" % - Initializing Forest Editor");
...
function ForestEditorPlugin::onWorldEditorStartup( %this )
{
    ...
    %managedItemDataPath = %missionPath @ "art/forest/managedItemData.cs";
    ...
    // make sure we recreate it freshly. Otherwise we will have duplicate items all over the place
    if ( isObject( ForestBrushGroup ) ) {
        ForestBrushGroup.delete();
    }

    %brushPath = %missionPath @ "/main.forestbrushes4.json";
    if ( isFile( %brushPath ) )
    {
        // load brushes in json
        LuaExecuteStringSync("Sim.deserializeObjectsFromStreamFile('" @ %brushPath @ "', true)");
    } else {
        // backward compatibility
        %brushPath = %missionPath @ "/main.forestbrushes.json";
        if ( isFile( %brushPath ) ) {
            // load brushes in json
            LuaExecuteStringSync("Sim.deserializeObjectsFromFile('" @ %brushPath @ "', true)");
        }
    }
...
function ForestEditorPlugin::onClientEndMission(%this) {
    ForestBrushGroup.delete();
}
...
function ForestEditorPlugin::onSaveMission( %this, %missionFile )
{
    ...
    LuaExecuteStringSync("Sim.serializeObjectToStreamFile('ForestBrushGroup','" @ getMissionPath() @ "/main.forestbrushes4.json')");
    //ForestBrushGroup.save( getMissionPath() @ "art/forest/brushes.cs" );
}
