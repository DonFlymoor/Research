...
function EWCreatorWindow::init( %this )
{
    ...
    %this.beginGroup( "Environment" );
        ...
        %this.registerMissionObject( "SFXEmitterLayer1D",   "Sound Emitter Layer1D" );
    ...
    %this.beginGroup( "Level" );
        ...
        %this.registerMissionObject( "SimPath", "Path" );
    ...
    %this.beginGroup( "BeamNG" );
        ...
        %this.registerMissionObject( "BeamNGVehicle", "Vehicle", "BeamNGVehicle" );
        %this.registerMissionObject( "BeamNGTrigger", "Lua Trigger", "BeamNGTrigger" );
        %this.registerMissionObject( "BeamNGWaypoint", "Waypoint", "BeamNGWaypoint" );
		%this.registerMissionObject( "BeamNGEnvTrigger", "Environment Trigger", "BeamNGEnvTrigger" );
        %this.registerMissionObject( "BeamNGPointOfInterest", "Point of Interest", "BeamNGPointOfInterest" );
        %this.registerMissionObject( "BeamNGGameplayArea", "Gameplay Area", "BeamNGGameplayArea" );
...
function EWCreatorWindow::onWake( %this )
{
    ...
    CreatorTabBook.onTabSelected( "Meshes" );
}
...
{
    %focusPoint = Game.getCameraHandler().getLookAtPoint();
    if( %focusPoint $= "" )
        return "0 0 0";
    else
...
function EWCreatorWindow::onFinishCreateObject( %this, %objId )
{
    ...
    if( %objId.isMemberOfClass( "SceneObject" ) )
    {
        ...
        //flush new position
        %objId.setTransformF( %objId.getTransformF() );
    }
...
function CreatorTabBook::onTabSelected( %this, %text, %idx )
{
    if ( %this.isAwake() )
    {
        ...
        %startSplitPath = "";
        if ( EWCreatorWindow.tab $= "Meshes" || EWCreatorWindow.tab $= "Prefabs") {            
            %missionFile = getMissionFilename();
            %startFolder = filePath(%missionFile);
            %startSplitPath = strreplace( %startFolder, "/", " " );
        }
        EWCreatorWindow.navigate( %startSplitPath );
    }
}
...
function EWCreatorWindow::navigate( %this, %address )
{
    ...
    if ( %this.tab $= "Scripted" )
    {
        ...
        for ( %i = 0; %i < %dataGroup.getCount(); %i++ )
        {
            ...
            // debug ("Obj: " @ %obj.getName() @ " - " @ %obj.category );
    ...
    if ( %this.tab $= "Meshes" )
    {
        %expr = getFormatExtensions();

        // directories
        if(true)
        {
            %directoryList = getDirectoryList( strreplace(%address, " ", "/"), 0 );
            %wordCount = getFieldCount( %directoryList );
            for( %i = 0; %i < %wordCount; %i++ )
            {
                %path = GetField( %directoryList, %i );
                %this.addFolderIcon( %path );
            }
        }

        %fullPath = findFirstFileMultiExpr( strreplace(%address, " ", "/") @ "/" @ %expr, 0 );

        while ( %fullPath !$= "" )
        {
            if (strstr(%fullPath, "cached.dts") != -1 || strstr(%fullPath, ".cdae") != -1)
            {
                %fullPath = findNextFileMultiExpr( %expr );
                continue;
            }
    ...
    if ( %this.tab $= "Prefabs" )
    {
        ...
        %expr = "*.prefab";
        %fullPath = findFirstFile( strreplace(%address, " ", "/") @ "/" @ %expr, 0 );
